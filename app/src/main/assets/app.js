...
/* ===== V1.5.61 - Embarque validado como fuente de datos para Inicio / Último / Embarques =====
   Regla:
   - Sin embarque validado: Último y Embarques quedan limpias.
   - Con embarque validado: Cliente, Origen y Destino se toman desde Firebase collection("embarque").
*/
const E61_VALID_EMB_KEY = "elta_validated_embarque_v1561";
let E61_VALID_EMB = null;
let E61_VALIDATE_TIMER = null;
let E61_VALIDATING = false;

function e61(id){ return document.getElementById(id); }
function e61Esc(v){ return escapeHtml(String(v ?? "")); }

function e61Store(v){
  E61_VALID_EMB = v || null;
  try{
    if(v) localStorage.setItem(E61_VALID_EMB_KEY, JSON.stringify(v));
    else localStorage.removeItem(E61_VALID_EMB_KEY);
  }catch(e){}
}

function e61Load(){
  if(E61_VALID_EMB) return E61_VALID_EMB;
  try{
    const v = JSON.parse(localStorage.getItem(E61_VALID_EMB_KEY) || "null");
    if(v && v.embarque) E61_VALID_EMB = v;
  }catch(e){}
  return E61_VALID_EMB;
}

function e61FirebaseReady(){
  try{
  ...
  try{
    return typeof firebaseReady === "function" ? firebaseReady() : false;
  }catch(e){}
  return false;
}

function e61EmbInputValue(){
  const el = e61("embarqueInput");
  return el ? el.value.trim() : "";
}

function e61NormalizeEmbDoc(id, x, requested){
  x = x || {};
  const emb = String(x.embarque || x.numero || id || requested || "").trim();
  return {
    id,
    embarque: emb,
    numero: String(x.numero || x.embarque || emb || "").trim(),
    cliente: String(x.cliente || x.cliente_nombre || x.customer || "").trim(),
    origen: String(x.origen || x.origen_nombre || x.origin || "").trim(),
    destino: String(x.destino || x.destino_nombre || x.destination || "").trim(),
    activo: x.activo !== false,
    raw: x
  };
}

async function e61BuscarEmbarqueFirebase(numero){
  const emb = String(numero || "").trim();
  if(!emb) return null;
  if(!e61FirebaseReady()) return null;
  ...
  for(const id of ids){
    try{
      ...
      if(d.exists) return e61NormalizeEmbDoc(d.id, d.data() || {}, emb);
    }catch(e){}
  }
  ...
  for(const field of ["embarque", "numero"]){
    try{
      ...
      if(!snap.empty){
        const d = snap.docs[0];
        return e61NormalizeEmbDoc(d.id, d.data() || {}, emb);
      }
    }catch(e){}
  }
...
function e61SetSelectByText(id, text){
  const sel = e61(id);
...
function e61EnsurePlaceholder(id, text){
  const sel = e61(id);
  if(!sel) return;
...
function e61ResetSelect(id, text){
  const sel = e61(id);
  if(!sel) return;
  e61EnsurePlaceholder(id, text);
  sel.disabled = false;
  sel.value = "";
  sel.selectedIndex = 0;
}

function e61ClearValidatedViews(){
  e61Store(null);

  e61ResetSelect("clienteSelect", "Ej: seleccione cliente...");
  e61ResetSelect("origenSelect", "Ej: seleccione origen...");
  e61ResetSelect("destinoSelect", "Ej: seleccione destino...");

  const info = e61("rutaInfo");
  if(info) info.innerHTML = "<b>Distancia:</b> -<br><b>Destino:</b> -";

  e61RenderEmbarqueClean();
  e61RenderUltimoClean();
}

function e61ApplyEmbarqueToInicio(v){
  if(!v) return;

  e61SetSelectByText("clienteSelect", v.cliente);
  e61SetSelectByText("origenSelect", v.origen);
  e61SetSelectByText("destinoSelect", v.destino);
  ...
  if(info){
    info.innerHTML =
      `<b>Distancia:</b> -<br>`+
      `<b>Destino:</b> ${e61Esc(v.destino || "-")}`;
  }
}

function e61EmbarqueHtml(v){
  return `<div class="embarqueItem open">`+
    `<div class="embTop"><b>Embarque ${e61Esc(v.embarque || v.numero || "-")}</b><span>Validado</span></div>`+
    `<div>Cliente: ${e61Esc(v.cliente || "-")}</div>`+
    `<div>Origen: ${e61Esc(v.origen || "-")}</div>`+
    `<div>Destino: ${e61Esc(v.destino || "-")}</div>`+
    `</div>`;
}

function e61RenderEmbarqueClean(){
  const titleIds = ["embarqueFiltro", "emb19title", "embarqueFiltro1519", "e60EmbTitle"];
  const listIds = ["embarqueList", "emb19list", "embarqueList1519", "e60EmbList", "embarquesList", "embarquesBox", "embarquesDetalle"];
  ...
  try{ window.__emb19Title = "-"; }catch(e){}
  try{ window.__emb19Good = ""; }catch(e){}
  try{ window.__tpodGoodEmbarquesHtml = ""; }catch(e){}
  try{ window.__tpodLastEmbarquesHtml = ""; }catch(e){}
}

function e61RenderEmbarqueValidated(v){
  const html = e61EmbarqueHtml(v);
  const title = v.embarque || v.numero || "-";

  ["embarqueFiltro", "emb19title", "embarqueFiltro1519", "e60EmbTitle"].forEach(id=>{
    const el = e61(id);
    if(el) el.innerText = title;
  });

  ["embarqueList", "emb19list", "embarqueList1519", "e60EmbList", "embarquesList", "embarquesBox", "embarquesDetalle"].forEach(id=>{
    const el = e61(id);
    if(el) el.innerHTML = html;
  });

  // Si no existe un contenedor visible, crear uno dentro de la sección.
  const sec = e61("embarque");
  if(sec && !e61("e61EmbList")){
    const card = document.createElement("div");
    card.className = "card e61EmbCard";
    card.innerHTML = `<div class="embarqueHeader"><b>Número Embarque</b><span id="e61EmbTitle">${e61Esc(title)}</span></div><div id="e61EmbList">${html}</div>`;
    sec.appendChild(card);
  }else{
    const t = e61("e61EmbTitle");
    const l = e61("e61EmbList");
    if(t) t.innerText = title;
    if(l) l.innerHTML = html;
  }

  try{ window.__emb19Title = title; }catch(e){}
...
function e61RenderUltimoClean(){
  ["lastBox", "ultimoList", "ultimoContent", "lastContent", "lastTransit", "ultimoBody"].forEach(id=>{
    const el = e61(id);
    if(el){
      if(id === "lastBox") el.innerText = "";
      else el.innerHTML = "";
    }
  });
}

function e61RenderUltimoValidated(v){
  const txt =
`Embarque validado

Embarque: ${v.embarque || v.numero || "-"}
Cliente: ${v.cliente || "-"}
Origen: ${v.origen || "-"}
Destino: ${v.destino || "-"}`;

  const box = e61("lastBox");
  ...
  ["ultimoList", "ultimoContent", "lastContent", "lastTransit", "ultimoBody"].forEach(id=>{
    ...
    if(el){
      el.innerHTML =
        ...
        `<div><b>Embarque:</b> ${e61Esc(v.embarque || v.numero || "-")}</div>`+
        `<div><b>Cliente:</b> ${e61Esc(v.cliente || "-")}</div>`+
        `<div><b>Origen:</b> ${e61Esc(v.origen || "-")}</div>`+
        `<div><b>Destino:</b> ${e61Esc(v.destino || "-")}</div>`+
        `</div>`;
    }
  });
}

async function e61ValidateCurrentEmbarque(){
  if(E61_VALIDATING) return e61Load();
  const emb = e61EmbInputValue();

  if(!emb){
    e61ClearValidatedViews();
    return null;
  }

  const currentStored = e61Load();
  if(currentStored && currentStored.embarque === emb){
    e61ApplyEmbarqueToInicio(currentStored);
    e61RenderEmbarqueValidated(currentStored);
    e61RenderUltimoValidated(currentStored);
    return currentStored;
  }

  E61_VALIDATING = true;
  try{
    const found = await e61BuscarEmbarqueFirebase(emb);
    ...
    e61Store(found);
    e61ApplyEmbarqueToInicio(found);
    e61RenderEmbarqueValidated(found);
    e61RenderUltimoValidated(found);
    return found;

  }finally{
    E61_VALIDATING = false;
  }
}

function e61DebounceValidate(){
  clearTimeout(E61_VALIDATE_TIMER);
  E61_VALIDATE_TIMER = setTimeout(()=>e61ValidateCurrentEmbarque(), 350);
}

/* Fuente única para las vistas */
currentEmbarqueValue = function(){
  const v = e61Load();
  return v && v.embarque ? v.embarque : "";
};

renderEmbarque = function(){
  const v = e61Load();
  if(v && v.embarque){
    e61RenderEmbarqueValidated(v);
  }else{
    e61RenderEmbarqueClean();
  }
};
...
selectedRoute = function(){
  ...
  const c = CLIENTES_DATA[e61("clienteSelect")?.value] || {};
  const o = ORIGENES_DATA[e61("origenSelect")?.value] || {};
  const d = DESTINOS_DATA[e61("destinoSelect")?.value] || {};
  return {
    cliente: c.cliente || "",
    origen: o.nombre || "",
    origen_lat: o.lat,
...
/* Validación obligatoria antes de iniciar */
const __e61OriginalIniciarTransito = typeof iniciarTransito === "function" ? iniciarTransito : null;
if(__e61OriginalIniciarTransito){
  iniciarTransito = async function(){
    const v = await e61ValidateCurrentEmbarque();
    if(!v){
      window.alert("Debe ingresar y validar un embarque existente en Firebase.");
      return;
    }
    return __e61OriginalIniciarTransito.apply(this, arguments);
  };
}

/* Reforzar al cambiar de vista */
const __e61Show = typeof show === "function" ? show : null;
if(__e61Show){
  show = function(id){
    __e61Show(id);
    setTimeout(()=>{
      ...
      if(id === "inicio"){
        const emb = e61EmbInputValue();
        if(emb) e61ValidateCurrentEmbarque();
        else e61ClearValidatedViews();
      }
    }, 120);
  };
}
...
document.addEventListener("DOMContentLoaded", ()=>{
  e61EnsurePlaceholder("clienteSelect", "Ej: seleccione cliente...");
  e61EnsurePlaceholder("origenSelect", "Ej: seleccione origen...");
  e61EnsurePlaceholder("destinoSelect", "Ej: seleccione destino...");

  const emb = e61("embarqueInput");
  if(emb){
    emb.addEventListener("input", ()=>{
      if(!emb.value.trim()) e61ClearValidatedViews();
      else e61DebounceValidate();
    });
    emb.addEventListener("change", e61DebounceValidate);
    emb.addEventListener("blur", ()=>e61ValidateCurrentEmbarque());
  }

  setTimeout(()=>{
    if(e61EmbInputValue()) e61ValidateCurrentEmbarque();
    else e61ClearValidatedViews();
  }, 800);
});

/* Antiparpadeo: si no hay embarque validado, mantener limpias Último y Embarques */
setInterval(()=>{
  const v = e61Load();
  if(!v || !v.embarque){
    e61RenderEmbarqueClean();
