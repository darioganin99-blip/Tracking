...
function show(id){
  if(id==="aduanas" && !(transit() && !transit().closed)){ updateAduanasTabState&&updateAduanasTabState(); return; }
  const views=["usuario","inicio","tracking","aduanas","embarque","alertas","clima","checklist","ultimo"];
  const buttons=["btn-usuario","btn-inicio","btn-tracking","btn-aduanas","btn-embarque","btn-alertas","btn-clima","btn-checklist","btn-ultimo"];
...
async function iniciarTransito(){
  ...
  if(abierto && !abierto.closed){
    try{ if(typeof hideStartBusyMini==="function") hideStartBusyMini(); }catch(e){}
  ...
  try{
    if(typeof showStartBusyMini==="function") showStartBusyMini();
    ...
    try{ if(typeof hideStartBusyMini==="function") hideStartBusyMini(); }catch(err){}
...
/* ===== V1.5.103 - Guardado Firebase antes de WhatsApp sin alterar mensaje ===== */
...
async function cerrarTransito(){
  ...
  try{
    ...
    t.closed=gps;
    await hidratarAduanas1598(t);

    let msg=await buildCierreMsgAsync(t);
    msg=anexarAduanas1598(msg,t);
...
function renderTrackingMap(t){
  ...
  async function firebaseReadyForMap(){
    // V1.5.103: tpodInitFirebase puede inicializar Firebase pero devolver undefined.
...
/* ===== V1.5.103 - Helper Aduanas compacto directo ===== */
...
function aduanaMs1598(r){
  const d=new Date((r&&(r.time||r.createdAt))||"");
  return isNaN(d.getTime())?0:d.getTime();
}
function aduanaFh1598(r){
...
function aduanaKey1598(r){
  return String((r&&(r.aduanaId||r.aduana))||"").trim().toLowerCase();
}
function aduanaMerge1598(a,b){
  const m={};
  (Array.isArray(a)?a:[]).concat(Array.isArray(b)?b:[]).forEach(r=>{
    if(!r)return;
    const k=[String(r.aduanaId||r.aduana||""),String(r.time||r.createdAt||""),String(r.embarque||""),String(r.flota||"")].join("|");
    m[k]=r;
  });
  return Object.values(m).sort((x,y)=>aduanaMs1598(x)-aduanaMs1598(y));
}
async function hidratarAduanas1598(t){
  if(!t)return t;
    ...
    if(typeof firebaseReady==="function" && firebaseReady()){
      ...
      if(transitId){
        try{
          ...
          regs=aduanaMerge1598(regs,s.docs.map(d=>({id:d.id,...(d.data()||{})})));
        }catch(e){}
      }
    }
  }catch(e){}
...
function bloqueAduanas1598(t){
  const regs=Array.isArray(t&&t.aduanas)?t.aduanas.slice():[];
  const m={};
  regs.forEach(r=>{
    const k=aduanaKey1598(r);
    if(!k)return;
    if(!m[k]||aduanaMs1598(r)>=aduanaMs1598(m[k]))m[k]=r;
  });
  const ult=Object.values(m).sort((a,b)=>aduanaMs1598(a)-aduanaMs1598(b));
  if(!ult.length)return "Sin registros";
  return ult.map(r=>`• ${aduanaTc1598(r.aduana||r.aduanaId||"Aduana")} - ${aduanaFh1598(r)}`).join("\n");
}
function limpiarAduanas1598(msg){
  return String(msg||"").replace(/\n*🛃 Aduanas:[\s\S]*$/i,"").replace(/\n*Aduanas:[\s\S]*$/i,"").trimEnd();
}
function anexarAduanas1598(msg,t){
  return limpiarAduanas1598(msg)+`\n\n🛃 Aduanas:\n${bloqueAduanas1598(t)}`;
}
...
/* ===== v1.5.103 VALIDACION EMBARQUE + ULTIMO ORIGINAL ===== */
...
/* ===== v1.5.103 COLECCION EMBARQUE + LIMPIEZA + COMPARTIDOS ===== */
...
/* ===== v1.5.103 FIX VALIDACION / ULTIMO / EMBARQUES ===== */
...
/* ===== v1.5.103 EMBARQUE DESTACADO + ULTIMO FORMATO ANTERIOR ===== */
...
/* ===== v1.5.103 ULTIMO FORMATO REFERENCIA + DEDUP EMBARQUES ===== */
...
/* ===== v1.5.103 ULTIMO FORMATO COMPLETO + EMBARQUES SOLO FLOTA ===== */
window.__tpodEmbarquesLoading=false;
window.__tpodLastEmbarquesHtml="";
...
/* ===== v1.5.103 EMBARQUE VALIDADO + ULTIMO COMPACTO ===== */
window.__tpodEmbarquesLoading=false;
window.__tpodLastEmbarquesHtml="";
...
/* ===== v1.5.103 TRACKING EMBARQUES POS FIX ===== */
window.__tpodEmbarquesLoading=false;
window.__tpodLastEmbarquesHtml="";
...
/* ===== v1.5.103 CERRAR APP EN USUARIO ===== */
...
/* ===== v1.5.103 CERRAR APP NATIVO + POSICION PRECISA EMBARQUE ===== */
function cerrarApp(){
  const ok=window.confirm("¿Desea salir de Track POD?");
  if(!ok)return;
  try{window.__tpodEmbarquesLoading=false;}catch(e){}
...
/* ===== v1.5.103 EMBARQUES ESTABLE + POSICION PRECISA ===== */
window.__tpodEmbarquesLoading=false;
...
/* ===== v1.5.103 EMBARQUES SIN LOADING + GPS ACTUAL ===== */
window.__tpodEmbarquesLoading=false;
window.__tpodLastEmbarquesHtml=window.__tpodLastEmbarquesHtml||"";
...
/* ===== v1.5.103 EMBARQUES ESTABLE FINAL ===== */
window.__tpodEmbarquesLoading=false;
window.__tpodLastEmbarquesHtml=window.__tpodLastEmbarquesHtml||"";
...
/* ===== v1.5.103 EMBARQUES ESTABLE + ULTIMO GPS ===== */
...
/* ===== v1.5.103 UBICACION UNICA WHATSAPP / EMBARQUES / ULTIMO ===== */
...
/* ===== v1.5.103 UBICACION WHATSAPP COMPARTIDA FINAL ===== */
...
/* ===== v1.5.103 EMBARQUES RENDER FINAL ===== */
...
/* ===== v1.5.103 SCROLL TOTAL EMBARQUES ===== */
...
/* ===== v1.5.103 GEO UNIFICADO LOCALIDAD PROVINCIA ===== */
...
/* ===== V1.5.103 - Habilitación Check List y alertas ===== */
...
/* ===== V1.5.103 - Normalizar visual botón Check List ===== */
...
/* ===== V1.5.103 - Inicio/Fin: Firebase combos + validar embarque =====
...
/* ===== V1.5.103 - Recuperar Tracking sin tocar otras vistas =====
   Alcance:
...
/* ===== V1.5.103 - UI sin distancias/tarjetas =====
   Cambios pedidos:
...
/* ===== V1.5.103 - optimización apertura =====
...
/* ===== V1.5.103 - Tracking: nunca mostrar ruta, sí referencias ===== */
...
/* ===== V1.5.103 - Tracking sin ruta azul definitivo ===== */
...
/* ===== V1.5.103 - FIX RAIZ coordenadas 0 en Inicio/Fin y Tracking ===== */
...
// Compact start dialog - no bloquea el click del botón
(function(){
  function ensureStartBusy(){
    if(document.getElementById('startBusyMini')) return;
    const d=document.createElement('div');
    d.id='startBusyMini';
    d.innerHTML='<div class="startBusyCard"><div class="startBusySpin"></div><div>Iniciando tránsito...</div></div>';
    document.body.appendChild(d);
  }

  window.showStartBusyMini=function(){
    ensureStartBusy();
    const e=document.getElementById('startBusyMini');
    if(e) e.style.display='flex';
  };

  window.hideStartBusyMini=function(){
    const e=document.getElementById('startBusyMini');
  ...
  function isStartButton(el){
    ...
    const on=((b.getAttribute&&b.getAttribute('onclick'))||'').toLowerCase();
    return t.includes('iniciar tránsito') || t.includes('iniciar transito') || on.includes('iniciartransito');
  }

  // Importante: no usar pointerdown/touchstart, porque el overlay aparece antes del click y puede bloquearlo.
  document.addEventListener('click', function(ev){
    if(!isStartButton(ev.target)) return;
    setTimeout(function(){
      try{ showStartBusyMini(); }catch(e){}
    },0);
  }, true);

  // Seguridad: si por cualquier motivo el proceso no lo cierra, no dejarlo infinito.
  window.startBusyMiniAutoClose=function(ms){
    setTimeout(function(){
      try{ hideStartBusyMini(); }catch(e){}
    }, ms || 30000);
  };
})();


// Cierre seguro del cartel al finalizar iniciarTransito, sin cambiar la lógica original
(function(){
  if(typeof iniciarTransito === 'function' && !window.__iniciarTransitoWrappedMini){
    window.__iniciarTransitoWrappedMini = true;
    const originalIniciarTransitoMini = iniciarTransito;
    iniciarTransito = async function(){
      try{
        const r = originalIniciarTransitoMini.apply(this, arguments);
...
/* ===== V1.5.103 - Vista Aduanas compacta confirmar orden ===== */
const ADUANA_ALERTAS_REMOVIDAS = [
  "Ingreso Aduana USP.",
  "Salida Aduana USP.",
  "Ingreso Aduana Los Andes",
  "Salida Aduana Los Andes",
  "Liberación Aduana"
];

function normalizePaisAduana(v){
  return String(v||"").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");
}

function aduanaDestinoNombre(t){
  const r=(t && t.route) || {};
  return String(r.destino || r.destination || "");
}

async function aduanaPaisDesdeDestino(t){
  const r=(t && t.route) || {};
  const directo = r.destino_pais || r.destinoPais || r.paisDestino || r.pais || "";
  if(directo) return String(directo);

  const destino = aduanaDestinoNombre(t);
  if(!destino) return "";

  try{
    if(!firebaseReady || !firebaseReady()) return "";
    const doc = await db.collection("destinos").doc(destino).get();
    if(doc.exists){
      const d = doc.data() || {};
      return String(d.pais || d.pasi || d.paisDestino || "");
    }
  }catch(e){}
  ...
  try{
    ...
    const nd = normalizePaisAduana(destino);
    for(const doc of snap.docs){
      const d = doc.data() || {};
      const names = [doc.id,d.nombre,d.name,d.destino]
        .map(x=>normalizePaisAduana(x))
        .filter(Boolean);
    ...
    console.log("No se pudo buscar país del destino", e);
  }
  return "";
}

function aduanaViewEnabled(){
  const t=transit();
  return !!(t && !t.closed);
}

function updateAduanasTabState(){
  const btn=$("btn-aduanas");
  if(!btn) return;
  const enabled=aduanaViewEnabled();
  btn.disabled=!enabled;
  btn.classList.toggle("tabDisabled",!enabled);
  btn.title=enabled ? "" : "Disponible con tránsito iniciado";

  // Estado visual correcto: sólo verde si la vista activa es Aduanas.
  const active = document.querySelector("section#aduanas") && !document.querySelector("section#aduanas").classList.contains("hidden");
  btn.classList.toggle("aduanasActive", !!active && enabled);
}

function filtrarAlertasAduana(){
  const sel=$("alertType");
  if(!sel) return;
  Array.from(sel.options).forEach(opt=>{
    if(ADUANA_ALERTAS_REMOVIDAS.includes(opt.textContent.trim())){
      opt.remove();
    }
  });
}

function aduanaOrdenValue(a){
  const raw = a.orden ?? a.order ?? a.ordengrupo ?? null;
  const n = Number(raw);
  return Number.isFinite(n) ? n : 999999;
}

function aduanaDisplayOrden(a, idx){
  const n = aduanaOrdenValue(a);
  return n === 999999 ? (idx + 1) : n;
}


async function cargarAduanasPais(pais){
  if(!pais) return [];
  if(!firebaseReady || !firebaseReady()) return [];

  const target=normalizePaisAduana(pais);
  try{
    const snap=await db.collection("aduana").get();
    return snap.docs.map(d=>({id:d.id,...(d.data()||{})}))
      ...
      .filter(a=>{
        ...
        return destinoAduana===target || destinoAduana.includes(target) || target.includes(destinoAduana);
      })
      .sort((a,b)=>{
        const oa = aduanaOrdenValue(a);
        const ob = aduanaOrdenValue(b);
        if(oa !== ob) return oa - ob;
        return String(a.id || a.nombre || "").localeCompare(String(b.id || b.nombre || ""));
      });
  }catch(e){
    console.log("No se pudieron leer aduanas",e);
    return [];
  }
}


function aduanaBusy(show,msg){
  let box=document.getElementById("aduanaBusyBox");
  if(show){
    if(!box){
      box=document.createElement("div");
      box.id="aduanaBusyBox";
      box.innerHTML='<div class="aduanaBusyCard"><div class="aduanaBusySpin"></div><div id="aduanaBusyText"></div></div>';
      document.body.appendChild(box);
    }
    const txt=document.getElementById("aduanaBusyText");
    if(txt) txt.textContent=msg||"Registrando ingreso...";
...
function aduanaConfirmarIngreso(nombre){
  return window.confirm("¿Confirmar registrar ingreso a " + nombre + "?");
}

async function registrarAduana(id,nombre){
  const t=transit();
  if(!t || t.closed){
  ...
  if(!aduanaConfirmarIngreso(nombre)){
    return;
  }

  aduanaBusy(true,"Registrando ingreso...");

  try{
    await new Promise(resolve=>setTimeout(resolve,0));
    const paisDestino = await aduanaPaisDesdeDestino(t);
    const gps=await getGps();
    const evento={
      tipo:"Ingreso Aduana",
      aduanaId:id,
      ...
      flota:t.user && t.user.fleet ? t.user.fleet : "",
      chofer:t.user && t.user.driver ? t.user.driver : "",
      destino:aduanaDestinoNombre(t),
      destinoPais:paisDestino
    };

    if(!t.aduanas) t.aduanas=[];
    t.aduanas.push(evento);
    save(LS.transit,t);
    saveTransitHistory(t);

    if(firebaseReady && firebaseReady()){
      await db.collection("aduana_registros").add({
        ...evento,
        transitId:t.id||"",
        createdAt:now()
      });
...
async function renderAduanas(){
  updateAduanasTabState();
  filtrarAlertasAduana();
  ...
  const destino=aduanaDestinoNombre(t);
  if(estado) estado.innerText= destino ? `Destino: ${destino}` : "Destino no informado.";

  list.innerHTML='<div class="emptyBox aduanaEmpty">Cargando aduanas...</div>';

  const pais=await aduanaPaisDesdeDestino(t);
  if(estado) estado.innerText = pais ? `Destino: ${destino} / País: ${pais}` : `Destino: ${destino} / País no informado.`;

  const aduanas=await cargarAduanasPais(pais);
  ...
  const regs=t.aduanas||[];
  list.innerHTML=aduanas.map((a,idx)=>{
    const rawId=String(a.id || a.nombre || a.aduana || "");
    const nombre=escapeHtml(rawId);
    const orden = aduanaDisplayOrden(a, idx);
    const ultimo=regs.filter(r=>String(r.aduanaId||r.aduana)===rawId).slice(-1)[0];
    const estadoHtml=ultimo ? `<div class="aduanaUltimo">Último: ${fmtDate(ultimo.time)}</div>` : "";
    return `<div class="aduanaItem">
      <div class="aduanaName">${orden}. ${nombre}</div>
      ${estadoHtml}
      <button class="btn aduanaBtn" onclick="registrarAduana('${escapeHtml(rawId).replace(/'/g,"&#039;")}','${nombre.replace(/'/g,"&#039;")}')">Registrar ingreso</button>
    </div>`;
  }).join("");
}

(function initAduanasVista(){
  filtrarAlertasAduana();

  const oldShow=typeof show==="function" ? show : null;
  if(oldShow && !window.__aduanasShowWrappedCompacto){
    window.__aduanasShowWrappedCompacto=true;
    show=function(id){
      const r=oldShow.apply(this,arguments);
      setTimeout(updateAduanasTabState,0);
      return r;
    };
  }

  const oldRenderInicio=typeof renderInicio==="function" ? renderInicio : null;
  if(oldRenderInicio && !window.__aduanasRenderInicioWrappedCompacto){
    window.__aduanasRenderInicioWrappedCompacto=true;
    renderInicio=function(){
      const r=oldRenderInicio.apply(this,arguments);
      updateAduanasTabState();
      return r;
    };
  }

  const oldCerrar=typeof cerrarTransito==="function" ? cerrarTransito : null;
  if(oldCerrar && !window.__aduanasCerrarWrappedCompacto){
    window.__aduanasCerrarWrappedCompacto=true;
    cerrarTransito=async function(){
      const r=await oldCerrar.apply(this,arguments);
      updateAduanasTabState();
      return r;
    };
  }
...
/* ===== V1.5.103 - Registros Aduana en vista, WhatsApp, cierre y último ===== */
...
function aduanaRegistroFecha1584(r){
  return fmtDateShort((r && (r.time || r.createdAt)) || "");
}

function aduanaRegistroUbicacion1584(r){
  const g = r && r.gps;
  if(!g || g.lat==null || g.lng==null) return "";
  return `${Number(g.lat).toFixed(5)}, ${Number(g.lng).toFixed(5)}`;
}

function aduanaRegistrosOrdenados1584(t){
  const arr = (t && t.aduanas && Array.isArray(t.aduanas)) ? t.aduanas.slice() : [];
  return arr.sort((a,b)=>new Date(a.time||a.createdAt||0)-new Date(b.time||b.createdAt||0));
}

function formatAduanasMultiline1584(t){
  const regs = aduanaRegistrosOrdenados1584(t);
  if(!regs.length) return "Sin registros";
  return regs.map(r=>{
    const nombre = aduanaTitleCaseMsg1584(r.aduana || r.aduanaId || "Aduana");
    const fecha = aduanaRegistroFecha1584(r);
    const gps = aduanaRegistroUbicacion1584(r);
    const gpsTxt = gps ? ` - GPS ${gps}` : "";
    return `• ${nombre} - ${fecha}${gpsTxt}`;
  }).join("\n");
}

function aduanaSection1584(t){
  return `\n\n🛃 Aduanas:\n${formatAduanasMultiline1584(t)}`;
}

function appendAduanasToMsg1584(msg,t){
  msg = String(msg || "");
  if(msg.includes("🛃 Aduanas:") || msg.includes("Aduanas:")) return msg;
  return msg + aduanaSection1584(t);
}

// WhatsApp actualización
const __aduanaBuildUpdateMsgAsync1584 = typeof buildUpdateMsgAsync === "function" ? buildUpdateMsgAsync : null;
if(__aduanaBuildUpdateMsgAsync1584){
  buildUpdateMsgAsync = async function(t){
    const msg = await __aduanaBuildUpdateMsgAsync1584.apply(this, arguments);
    return appendAduanasToMsg1584(msg, t);
  };
}
const __aduanaBuildUpdateMsg1584 = typeof buildUpdateMsg === "function" ? buildUpdateMsg : null;
if(__aduanaBuildUpdateMsg1584){
  buildUpdateMsg = function(t){
    const msg = __aduanaBuildUpdateMsg1584.apply(this, arguments);
    return appendAduanasToMsg1584(msg, t);
  };
}

// WhatsApp cierre
const __aduanaBuildCierreMsgAsync1584 = typeof buildCierreMsgAsync === "function" ? buildCierreMsgAsync : null;
if(__aduanaBuildCierreMsgAsync1584){
  buildCierreMsgAsync = async function(t){
    const msg = await __aduanaBuildCierreMsgAsync1584.apply(this, arguments);
    return appendAduanasToMsg1584(msg, t);
  };
}
const __aduanaBuildCierreMsg1584 = typeof buildCierreMsg === "function" ? buildCierreMsg : null;
if(__aduanaBuildCierreMsg1584){
  buildCierreMsg = function(t){
    const msg = __aduanaBuildCierreMsg1584.apply(this, arguments);
    return appendAduanasToMsg1584(msg, t);
  };
}

// Guardado de último: si hay tránsito activo/cerrado, asegurar sección Aduanas antes de guardar/enviar.
const __aduanaSendToPhones1584 = typeof sendToPhones === "function" ? sendToPhones : null;
if(__aduanaSendToPhones1584){
  sendToPhones = function(msg){
    let t = null;
    try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
    const finalMsg = t ? appendAduanasToMsg1584(msg,t) : msg;
    return __aduanaSendToPhones1584.call(this, finalMsg);
  };
}

// Último: mostrar Aduanas si el mensaje ya lo tiene; si no, intenta anexar desde el tránsito local.
const __aduanaRenderUltimo1584 = typeof renderUltimo === "function" ? renderUltimo : null;
if(__aduanaRenderUltimo1584){
  renderUltimo = function(){
    const last = load(LS.last,null);
    if(last && last.msg && !String(last.msg).includes("Aduanas")){
      try{
        const t = typeof transit === "function" ? transit() : null;
        if(t && t.aduanas && t.aduanas.length){
          last.msg = appendAduanasToMsg1584(last.msg,t);
          save(LS.last,last);
        }
      }catch(e){}
    }
    return __aduanaRenderUltimo1584.apply(this, arguments);
  };
}

// Vista Aduanas: al cargar, si la aduana ya fue registrada, mostrar el dato de registro.
const __aduanaRenderAduanas1584 = typeof renderAduanas === "function" ? renderAduanas : null;
if(__aduanaRenderAduanas1584){
  renderAduanas = async function(){
    await __aduanaRenderAduanas1584.apply(this, arguments);
    ...
    try{
      ...
      document.querySelectorAll(".aduanaItem").forEach(item=>{
        ...
        const regs = aduanaRegistrosOrdenados1584(t).filter(r=>{
          const n = aduanaTitleCaseMsg1584(r.aduana || r.aduanaId || "").toLowerCase();
          return n && visible.includes(n);
        });
        if(!regs.length) return;
        const ultimo = regs[regs.length-1];
        let info = item.querySelector(".aduanaRegistroDato1584");
        if(!info){
          info = document.createElement("div");
          info.className = "aduanaRegistroDato1584";
          nameEl.insertAdjacentElement("afterend", info);
        }
        const gps = aduanaRegistroUbicacion1584(ultimo);
        info.textContent = gps
          ? `Registrado: ${aduanaRegistroFecha1584(ultimo)} / GPS ${gps}`
          : `Registrado: ${aduanaRegistroFecha1584(ultimo)}`;
        const btn = item.querySelector(".aduanaBtn");
        if(btn) btn.textContent = "Registrar nuevo ingreso";
      });
    }catch(e){
      console.log("No se pudo pintar registros de aduana", e);
    }
  };
}


/* ===== V1.5.103 - Aduanas reflejo directo en vista, WhatsApp y último ===== */
function aduanaTitleCaseMsg1585(v){
  if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
  return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
}

function aduanaRegistroFecha1585(r){
  return fmtDateShort((r && (r.time || r.createdAt)) || "");
}

function aduanaRegistroGps1585(r){
  const g = r && r.gps;
  if(!g || g.lat==null || g.lng==null) return "";
  return `${Number(g.lat).toFixed(5)}, ${Number(g.lng).toFixed(5)}`;
}

function aduanaRegs1585(t){
  return ((t && Array.isArray(t.aduanas)) ? t.aduanas.slice() : [])
    .sort((a,b)=>new Date(a.time||a.createdAt||0)-new Date(b.time||b.createdAt||0));
}

function formatAduanasMultiline1585(t){
  const regs = aduanaRegs1585(t);
  if(!regs.length) return "Sin registros";
  return regs.map(r=>{
    const nombre = aduanaTitleCaseMsg1585(r.aduana || r.aduanaId || "Aduana");
    const fecha = aduanaRegistroFecha1585(r);
    const gps = aduanaRegistroGps1585(r);
    return `• ${nombre} - ${fecha}${gps ? " - GPS " + gps : ""}`;
  }).join("\n");
}

function appendAduanasToMsg1585(msg,t){
  msg = String(msg || "");
  msg = msg.replace(/\n*🛃 Aduanas:[\s\S]*$/,"").trimEnd();
  return msg + `\n\n🛃 Aduanas:\n${formatAduanasMultiline1585(t)}`;
}

/* Reemplazo directo de mensajes para que sí se refleje */
if(typeof buildUpdateMsgAsync === "function"){
  const __oldBuildUpdateMsgAsync1585 = buildUpdateMsgAsync;
  buildUpdateMsgAsync = async function(t){
    const msg = await __oldBuildUpdateMsgAsync1585.apply(this, arguments);
    return appendAduanasToMsg1585(msg,t);
  };
}

if(typeof buildUpdateMsg === "function"){
  const __oldBuildUpdateMsg1585 = buildUpdateMsg;
  buildUpdateMsg = function(t){
    const msg = __oldBuildUpdateMsg1585.apply(this, arguments);
    return appendAduanasToMsg1585(msg,t);
  };
}

if(typeof buildCierreMsgAsync === "function"){
  const __oldBuildCierreMsgAsync1585 = buildCierreMsgAsync;
  buildCierreMsgAsync = async function(t){
    const msg = await __oldBuildCierreMsgAsync1585.apply(this, arguments);
    return appendAduanasToMsg1585(msg,t);
  };
}

if(typeof buildCierreMsg === "function"){
  const __oldBuildCierreMsg1585 = buildCierreMsg;
  buildCierreMsg = function(t){
    const msg = __oldBuildCierreMsg.apply(this, arguments);
    return appendAduanasToMsg1585(msg,t);
  };
}

/* Envío: asegurar que el último guardado y el WhatsApp lleven Aduanas */
if(typeof sendToPhones === "function"){
  const __oldSendToPhones1585 = sendToPhones;
  sendToPhones = function(msg){
    let t = null;
    try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
    const finalMsg = t ? appendAduanasToMsg1585(msg,t) : msg;
    return __oldSendToPhones1585.call(this, finalMsg);
  };
}

/* Último registro: mostrar el mensaje con Aduanas si hay tránsito local */
if(typeof renderUltimo === "function"){
  const __oldRenderUltimo1585 = renderUltimo;
  renderUltimo = function(){
    let last = load(LS.last,null);
    try{
      const t = typeof transit === "function" ? transit() : null;
      if(last && last.msg && t){
        last.msg = appendAduanasToMsg1585(last.msg,t);
        save(LS.last,last);
      }
    }catch(e){}
    return __oldRenderUltimo1585.apply(this, arguments);
  };
}

/* Vista Aduanas: pintar registro existente apenas carga la vista */
function pintarRegistrosAduana1585(){
  try{
    const t = transit();
    if(!t || !t.aduanas || !t.aduanas.length) return;
    ...
    document.querySelectorAll(".aduanaItem").forEach(item=>{
      ...
      const visible = String(nameEl.textContent || "").replace(/^\s*\d+\.\s*/,"").trim().toLowerCase();
      const regs = aduanaRegs1585(t).filter(r=>{
        const n = aduanaTitleCaseMsg1585(r.aduana || r.aduanaId || "").toLowerCase();
      ...
      let info = item.querySelector(".aduanaRegistroDato1585");
      if(!info){
        info = document.createElement("div");
        info.className = "aduanaRegistroDato1585";
        nameEl.insertAdjacentElement("afterend", info);
      }

      const gps = aduanaRegistroGps1585(ultimo);
      info.textContent = gps
        ? `Registro de aduana: ${aduanaRegistroFecha1585(ultimo)} / GPS ${gps}`
        : `Registro de aduana: ${aduanaRegistroFecha1585(ultimo)}`;

      const btn = item.querySelector(".aduanaBtn");
      if(btn) btn.textContent = "Registrar nuevo ingreso";
    });
  }catch(e){
    console.log("No se pudo pintar registro de aduana", e);
  }
}

if(typeof renderAduanas === "function"){
  const __oldRenderAduanas1585 = renderAduanas;
  renderAduanas = async function(){
    await __oldRenderAduanas1585.apply(this, arguments);
    setTimeout(pintarRegistrosAduana1585,50);
  };
}


/* ===== V1.5.103 - Aduanas desde Firebase en vista y WhatsApp ===== */
function aduanaTitleCaseMsg1586(v){
  if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
  return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
}

function aduanaRegistroFecha1586(r){
  return fmtDateShort((r && (r.time || r.createdAt)) || "");
}

function aduanaRegistroGps1586(r){
  const g = r && r.gps;
  if(!g || g.lat==null || g.lng==null) return "";
  return `${Number(g.lat).toFixed(5)}, ${Number(g.lng).toFixed(5)}`;
}

function aduanaKey1586(r){
  return [
...
function mergeAduanaRegs1586(a,b){
  const map = {};
  (Array.isArray(a)?a:[]).concat(Array.isArray(b)?b:[]).forEach(r=>{
    if(!r) return;
    map[aduanaKey1586(r)] = r;
  });
  return Object.values(map).sort((x,y)=>new Date(x.time||x.createdAt||0)-new Date(y.time||y.createdAt||0));
}

async function hydrateAduanasTransito1586(t){
  if(!t) return t;
  let regs = Array.isArray(t.aduanas) ? t.aduanas.slice() : [];
  ...
  try{
    if(firebaseReady && firebaseReady()){
      ...
      if(transitId){
        try{
          const snap = await db.collection("aduana_registros").where("transitId","==",transitId).get();
          regs = mergeAduanaRegs1586(regs, snap.docs.map(d=>({id:d.id,...(d.data()||{})})));
        }catch(e){}
      }

      if(embarque && flota){
        try{
          const snap2 = await db.collection("aduana_registros").where("embarque","==",embarque).where("flota","==",flota).get();
          regs = mergeAduanaRegs1586(regs, snap2.docs.map(d=>({id:d.id,...(d.data()||{})})));
        }catch(e){}
      }

      if(!regs.length && embarque){
        try{
          const snap3 = await db.collection("aduana_registros").where("embarque","==",embarque).get();
          const all = snap3.docs.map(d=>({id:d.id,...(d.data()||{})}));
          regs = mergeAduanaRegs1586(regs, all.filter(r=>!flota || String(r.flota||"")===flota));
        }catch(e){}
      }
    }
  }catch(e){
    console.log("No se pudieron hidratar registros de aduana", e);
  }

  if(regs.length){
    t.aduanas = regs;
    try{
      const current = typeof transit === "function" ? transit() : null;
...
function aduanaRegs1586(t){
  return ((t && Array.isArray(t.aduanas)) ? t.aduanas.slice() : [])
    .sort((a,b)=>new Date(a.time||a.createdAt||0)-new Date(b.time||b.createdAt||0));
}

function formatAduanasMultiline1586(t){
  const regs = aduanaRegs1586(t);
  if(!regs.length) return "Sin registros";
  return regs.map(r=>{
    const nombre = aduanaTitleCaseMsg1586(r.aduana || r.aduanaId || "Aduana");
    const fecha = aduanaRegistroFecha1586(r);
    const gps = aduanaRegistroGps1586(r);
    return `• ${nombre} - ${fecha}${gps ? " - GPS " + gps : ""}`;
  }).join("\n");
}

function appendAduanasToMsg1586(msg,t){
  msg = String(msg || "");
  msg = msg.replace(/\n*🛃 Aduanas:[\s\S]*$/,"").trimEnd();
  return msg + `\n\n🛃 Aduanas:\n${formatAduanasMultiline1586(t)}`;
}

/* Mensajes WhatsApp con hidratación previa */
if(typeof buildUpdateMsgAsync === "function"){
  const __oldBuildUpdateMsgAsync1586 = buildUpdateMsgAsync;
  buildUpdateMsgAsync = async function(t){
    t = await hydrateAduanasTransito1586(t);
    const msg = await __oldBuildUpdateMsgAsync1586.apply(this, arguments);
    return appendAduanasToMsg1586(msg,t);
  };
}

if(typeof buildUpdateMsg === "function"){
  const __oldBuildUpdateMsg1586 = buildUpdateMsg;
  buildUpdateMsg = function(t){
    const msg = __oldBuildUpdateMsg1586.apply(this, arguments);
    return appendAduanasToMsg1586(msg,t);
  };
}

if(typeof buildCierreMsgAsync === "function"){
  const __oldBuildCierreMsgAsync1586 = buildCierreMsgAsync;
  buildCierreMsgAsync = async function(t){
    t = await hydrateAduanasTransito1586(t);
    const msg = await __oldBuildCierreMsgAsync1586.apply(this, arguments);
    return appendAduanasToMsg1586(msg,t);
  };
}

if(typeof buildCierreMsg === "function"){
  const __oldBuildCierreMsg1586 = buildCierreMsg;
  buildCierreMsg = function(t){
    const msg = __oldBuildCierreMsg1586.apply(this, arguments);
    return appendAduanasToMsg1586(msg,t);
  };
}

/* Enviar actualización: asegurar que t.aduanas esté actualizado antes de armar mensaje */
if(typeof enviarActualizacion === "function"){
  const __oldEnviarActualizacion1586 = enviarActualizacion;
  enviarActualizacion = async function(){
    try{
      const t = typeof transit === "function" ? transit() : null;
      if(t){
        await hydrateAduanasTransito1586(t);
      }
    }catch(e){}
    return __oldEnviarActualizacion1586.apply(this, arguments);
  };
}

/* Último registro */
if(typeof renderUltimo === "function"){
  const __oldRenderUltimo1586 = renderUltimo;
  renderUltimo = async function(){
    try{
      let last = load(LS.last,null);
      const t = typeof transit === "function" ? transit() : null;
      if(last && last.msg && t){
        await hydrateAduanasTransito1586(t);
        last.msg = appendAduanasToMsg1586(last.msg,t);
        save(LS.last,last);
      }
    }catch(e){}
    return __oldRenderUltimo1586.apply(this, arguments);
  };
}

/* Vista Aduanas: hidratar desde Firebase y pintar registro existente */
function pintarRegistrosAduana1586(){
  try{
    const t = transit();
    if(!t || !t.aduanas || !t.aduanas.length) return;
    ...
    document.querySelectorAll(".aduanaItem").forEach(item=>{
      ...
      const visible = String(nameEl.textContent || "").replace(/^\s*\d+\.\s*/,"").trim().toLowerCase();
      const regs = aduanaRegs1586(t).filter(r=>{
        const n = aduanaTitleCaseMsg1586(r.aduana || r.aduanaId || "").toLowerCase();
        return n && (visible.includes(n) || n.includes(visible));
      });
      ...
      let info = item.querySelector(".aduanaRegistroDato1586");
      if(!info){
        info = document.createElement("div");
        info.className = "aduanaRegistroDato1586";
        nameEl.insertAdjacentElement("afterend", info);
      }

      const gps = aduanaRegistroGps1586(ultimo);
      info.textContent = gps
        ? `Registro de aduana: ${aduanaRegistroFecha1586(ultimo)} / GPS ${gps}`
        : `Registro de aduana: ${aduanaRegistroFecha1586(ultimo)}`;
...
if(typeof renderAduanas === "function"){
  const __oldRenderAduanas1586 = renderAduanas;
  renderAduanas = async function(){
    const t = typeof transit === "function" ? transit() : null;
    if(t) await hydrateAduanasTransito1586(t);
    await __oldRenderAduanas1586.apply(this, arguments);
    setTimeout(pintarRegistrosAduana1586,50);
    setTimeout(pintarRegistrosAduana1586,300);
  };
}

/* Al guardar registro, volver a pintar con datos locales y Firebase */
if(typeof registrarAduana === "function"){
  const __oldRegistrarAduana1586 = registrarAduana;
  registrarAduana = async function(){
    const r = await __oldRegistrarAduana1586.apply(this, arguments);
    try{
      const t = typeof transit === "function" ? transit() : null;
      if(t) await hydrateAduanasTransito1586(t);
      setTimeout(pintarRegistrosAduana1586,100);
    }catch(e){}
    return r;
  };
}
...
/* ===== V1.5.103 - Aduanas compacto mensaje y vista ===== */
...
/* ===== V1.5.103 - Aduanas FINAL ultimo por aduana ===== */
(function(){
...
/* ===== V1.5.103 - Aduanas unificado en actualización, cierre y último ===== */
(function(){
  function titleCase(v){
    if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
    return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
  ...
  async function hidratar(t){
    if(typeof window.__aduanaHidratarFinal1588 === "function"){
      return await window.__aduanaHidratarFinal1588(t);
    }
    if(typeof hydrateAduanasTransito1586 === "function"){
      return await hydrateAduanasTransito1586(t);
    }
    return t;
  }

  function bloqueCompacto(t){
    const regs = ultimosPorAduana(t);
    if(!regs.length) return "Sin registros";
    return regs.map(r => `• ${titleCase(r.aduana || r.aduanaId || "Aduana")} - ${fechaHora(r)}`).join("\n");
  }

  function limpiarBloqueAduanas(msg){
    return String(msg || "")
      .replace(/\n*🛃 Aduanas:[\s\S]*$/,"")
      .replace(/\n*Aduanas:[\s\S]*$/,"")
      .trimEnd();
  ...
  function anexarCompacto(msg,t){
    return limpiarBloqueAduanas(msg) + `\n\n🛃 Aduanas:\n${bloqueCompacto(t)}`;
  }

  window.__aduanaMsgCompactoFinal1589 = anexarCompacto;

  async function mensajeActualizacionUnificado(t){
    t = await hidratar(t);
    let base = "";
    if(typeof buildUpdateMsgAsync === "function"){
      base = await buildUpdateMsgAsync(t);
    ...
    return anexarCompacto(base,t);
  }

  async function mensajeCierreUnificado(t){
    t = await hidratar(t);
    let base = "";
    if(typeof buildCierreMsgAsync === "function"){
      base = await buildCierreMsgAsync(t);
    }else if(typeof buildCierreMsg === "function"){
      base = buildCierreMsg(t);
    }else{
      base = "🏁 Cierre de tránsito";
    }
    return anexarCompacto(base,t);
  }

  // Actualización: reemplazo final directo para evitar que vuelva a salir otro formato.
  window.enviarActualizacion = async function(){
    const t = typeof transit === "function" ? transit() : null;
    if(!t){
      window.alert("No hay tránsito iniciado.");
      return;
    ...
    try{
      const msg = await mensajeActualizacionUnificado(t);
      save(LS.last,{msg,date:now()});
  ...
  // Cierre: el mensaje guardado/enviado debe quedar con el mismo bloque compacto.
  if(typeof buildCierreMsgAsync === "function" && !window.__aduanaCierreUnificado1589){
    window.__aduanaCierreUnificado1589 = true;
    const oldAsync = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      t = await hidratar(t);
      const base = await oldAsync.apply(this, arguments);
      return anexarCompacto(base,t);
    };
  }

  if(typeof buildCierreMsg === "function" && !window.__aduanaCierreSyncUnificado1589){
    window.__aduanaCierreSyncUnificado1589 = true;
    const oldSync = buildCierreMsg;
    buildCierreMsg = function(t){
      const base = oldSync.apply(this, arguments);
      return anexarCompacto(base,t);
    };
  }

  // Envío genérico: si el mensaje es actualización, cierre o último, normalizar bloque Aduanas.
  if(typeof sendToPhones === "function" && !window.__aduanaSendUnificado1589){
    window.__aduanaSendUnificado1589 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      let t = null;
      try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
      const finalMsg = t ? anexarCompacto(msg,t) : limpiarBloqueAduanas(msg);
      return oldSend.call(this, finalMsg);
    };
  }

  // Último registro: siempre mostrar el mismo formato compacto.
  if(typeof renderUltimo === "function" && !window.__aduanaUltimoUnificado1589){
    window.__aduanaUltimoUnificado1589 = true;
    const oldUltimo = renderUltimo;
    renderUltimo = async function(){
      try{
        let last = load(LS.last,null);
        const t = typeof transit === "function" ? transit() : null;
        if(last && last.msg && t){
          await hidratar(t);
          last.msg = anexarCompacto(last.msg,t);
          save(LS.last,last);
        }
      }catch(e){}
      return oldUltimo.apply(this, arguments);
    };
  }

  // Vista Aduanas: mantener también último por aduana en pantalla.
  function pintarUltimosVista(){
    try{
      const t = typeof transit === "function" ? transit() : null;
      if(!t || !Array.isArray(t.aduanas) || !t.aduanas.length) return;

      const ultimos = ultimosPorAduana(t);
      document.querySelectorAll(".aduanaItem").forEach(item=>{
        const nameEl = item.querySelector(".aduanaName");
        if(!nameEl) return;
        const visible = String(nameEl.textContent || "").replace(/^\s*\d+\.\s*/,"").trim().toLowerCase();
        ...
        item.querySelectorAll(".aduanaRegistroDato1584,.aduanaRegistroDato1585,.aduanaRegistroDato1586,.aduanaRegistroDato1587,.aduanaRegistroDato1588,.aduanaRegistroDato1589").forEach(e=>e.remove());
        const info = document.createElement("div");
        info.className = "aduanaRegistroDato1589";
        info.textContent = fechaHora(r);
        nameEl.insertAdjacentElement("afterend", info);
  ...
  if(typeof renderAduanas === "function" && !window.__aduanaVistaUnificado1589){
    window.__aduanaVistaUnificado1589 = true;
    const oldAduanas = renderAduanas;
    renderAduanas = async function(){
      const t = typeof transit === "function" ? transit() : null;
      if(t) await hidratar(t);
      const r = await oldAduanas.apply(this, arguments);
      setTimeout(pintarUltimosVista,60);
      setTimeout(pintarUltimosVista,300);
      return r;
    };
  }
})();
...
/* ===== V1.5.103 - Aduanas sin texto Ultimo ===== */
(function(){
  function limpiarTextoUltimoAduanas1590(){
    try{
      document.querySelectorAll(".aduanaUltimo").forEach(e=>e.remove());
      document.querySelectorAll(".aduanaRegistroDato1584,.aduanaRegistroDato1585,.aduanaRegistroDato1586,.aduanaRegistroDato1587,.aduanaRegistroDato1588,.aduanaRegistroDato1589").forEach(e=>{
        e.textContent = String(e.textContent || "")
          .replace(/^Último:\s*/i,"")
          .replace(/^Ultimo:\s*/i,"")
          .replace(/^Registro de aduana:\s*/i,"")
          .replace(/\s*\/\s*GPS.*$/i,"")
          .trim();
      });
    }catch(e){}
  ...
  if(typeof renderAduanas === "function" && !window.__aduanasSinUltimo1590){
    window.__aduanasSinUltimo1590 = true;
    const oldRenderAduanas1590 = renderAduanas;
    renderAduanas = async function(){
      const r = await oldRenderAduanas1590.apply(this, arguments);
      setTimeout(limpiarTextoUltimoAduanas1590,80);
      setTimeout(limpiarTextoUltimoAduanas1590,350);
      return r;
    };
  }

  document.addEventListener("DOMContentLoaded",()=>{
    setTimeout(limpiarTextoUltimoAduanas1590,500);
  });
})();


/* ===== V1.5.103 - Aduanas única lógica compacta ===== */
(function(){
  function titleCase(v){
    if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
    return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
  ...
  function mergeRegs(a,b){
    ...
    return Object.values(m).sort((x,y)=>fechaMs(x)-fechaMs(y));
  }
  ...
  async function hydrate(t){
    ...
    }catch(e){ console.log("Aduanas hydrate", e); }
  ...
  function ultimosPorAduana(t){
    ...
    regs.forEach(r=>{
      const k = key(r);
      if(!k) return;
      if(!m[k] || fechaMs(r) >= fechaMs(m[k])) m[k] = r;
    });
    return Object.values(m).sort((a,b)=>fechaMs(a)-fechaMs(b));
  }
  ...
  window.buildCompactAduanas1591 = function(t){ return bloque(t); };
  window.appendCompactAduanas1591 = function(msg,t){ return anexar(msg,t); };
  window.hydrateAduanas1591 = hydrate;

  async function buildUpdateFinal(t){
    t = await hydrate(t);
    let base = "";
    if(typeof buildUpdateMsgAsync === "function") base = await buildUpdateMsgAsync(t);
    else if(typeof buildUpdateMsg === "function") base = buildUpdateMsg(t);
    else base = "🚚 Actualización de tránsito";
    return anexar(base,t);
  }

  async function buildCierreFinal(t){
    t = await hydrate(t);
    let base = "";
    if(typeof buildCierreMsgAsync === "function") base = await buildCierreMsgAsync(t);
    else if(typeof buildCierreMsg === "function") base = buildCierreMsg(t);
    else base = "🏁 Cierre de tránsito";
    return anexar(base,t);
  }
  ...
  window.enviarActualizacion = async function(){
    ...
    try{
      const msg = await buildUpdateFinal(t);
      save(LS.last,{msg,date:now()});
      const u = typeof user === "function" ? user() : {};
      const phones = String(u.phones||"").split(/[,;\n\r]+/).map(cleanPhone).filter(Boolean);
      const text = encodeURIComponent(msg);
      ...
      if(btn){ btn.disabled = false; btn.innerText = "📤 Enviar actualización"; }
    }
  };

  if(typeof buildCierreMsgAsync === "function" && !window.__cierreAduanas1591){
    window.__cierreAduanas1591 = true;
    const old = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      t = await hydrate(t);
      const base = await old.apply(this, arguments);
  ...
  if(typeof buildCierreMsg === "function" && !window.__cierreAduanasSync1591){
    window.__cierreAduanasSync1591 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){
      const base = old.apply(this, arguments);
      return anexar(base,t);
  ...
  if(typeof cerrarTransito === "function" && !window.__cerrarTransitoAduanas1591){
    window.__cerrarTransitoAduanas1591 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      try{
        const t = typeof transit === "function" ? transit() : null;
        if(t) await hydrate(t);
      }catch(e){}
      return await oldCerrar.apply(this, arguments);
    };
  }

  if(typeof sendToPhones === "function" && !window.__sendAduanas1591){
    window.__sendAduanas1591 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      let t = null;
      try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
      const finalMsg = t ? anexar(msg,t) : limpiar(msg);
      try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){}
      return oldSend.call(this, finalMsg);
    };
  }

  if(typeof renderUltimo === "function" && !window.__ultimoAduanas1591){
    window.__ultimoAduanas1591 = true;
    const oldUltimo = renderUltimo;
    renderUltimo = async function(){
      try{
        let last = load(LS.last,null);
  ...
  function pintarVista(){
    try{
      ...
      const ultimos = ultimosPorAduana(t);
      document.querySelectorAll(".aduanaItem").forEach(item=>{
        item.querySelectorAll(".aduanaUltimo,.aduanaRegistroDato1584,.aduanaRegistroDato1585,.aduanaRegistroDato1586,.aduanaRegistroDato1587,.aduanaRegistroDato1588,.aduanaRegistroDato1589,.aduanaRegistroDato1591").forEach(e=>e.remove());
        ...
        const info = document.createElement("div");
        info.className = "aduanaRegistroDato1591";
        info.textContent = fechaHora(r);
        nameEl.insertAdjacentElement("afterend", info);
  ...
  if(typeof renderAduanas === "function" && !window.__vistaAduanas1591){
    window.__vistaAduanas1591 = true;
    const oldAduanas = renderAduanas;
    renderAduanas = async function(){
      const t = typeof transit === "function" ? transit() : null;
      if(t) await hydrate(t);
      const r = await oldAduanas.apply(this, arguments);
      setTimeout(pintarVista,60);
      setTimeout(pintarVista,300);
      return r;
    };
  }
})();
...
/* ===== V1.5.103 - Aduanas definitivo sin GPS ===== */
(function(){
  function title(v){
    if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
    return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
  ...
  function merge(a,b){
    ...
    return Object.values(m).sort((x,y)=>ms(x)-ms(y));
  }
  async function hydrate(t){
    if(!t) return t;
    let regs = Array.isArray(t.aduanas) ? t.aduanas.slice() : [];
    try{
      if(typeof firebaseReady === "function" && firebaseReady()){
        ...
        if(transitId){
          try{
            const s = await db.collection("aduana_registros").where("transitId","==",transitId).get();
            regs = merge(regs, s.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
        if(embarque && flota){
          try{
            const s2 = await db.collection("aduana_registros").where("embarque","==",embarque).where("flota","==",flota).get();
            regs = merge(regs, s2.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
      }
    }catch(e){}
  ...
  function bloque(t){
    const regs = ultimos(t);
    if(!regs.length) return "Sin registros";
    return regs.map(r => `• ${title(r.aduana || r.aduanaId || "Aduana")} - ${fh(r)}`).join("\n");
  }
  function limpiar(msg){
    return String(msg || "")
      .replace(/\n*🛃 Aduanas:[\s\S]*$/i,"")
  ...
  function normalizar(msg,t){
    return limpiar(msg) + `\n\n🛃 Aduanas:\n${bloque(t)}`;
  }
  function normalizarActual(msg){
    let t = null;
    try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
    return t ? normalizar(msg,t) : limpiar(msg);
  }

  window.aduanasCompactoDef1592 = { hydrate, bloque, normalizar };

  async function buildUpdateBase(t){
    let base = "";
    if(typeof buildUpdateMsgAsync === "function") base = await buildUpdateMsgAsync(t);
    else if(typeof buildUpdateMsg === "function") base = buildUpdateMsg(t);
    else base = "🚚 Actualización de tránsito";
  ...
  window.enviarActualizacion = async function(){
    const t0 = typeof transit === "function" ? transit() : null;
    if(!t0){ window.alert("No hay tránsito iniciado."); return; }
    const btn = document.querySelector('button[onclick="enviarActualizacion()"]');
    if(btn){ btn.disabled = true; btn.innerText = "Enviando..."; }
    try{
      const t = await hydrate(t0);
      const msg = normalizar(await buildUpdateBase(t),t);
      save(LS.last,{msg,date:now()});
      const u = typeof user === "function" ? user() : {};
      const phones = String(u.phones||"").split(/[,;\n\r]+/).map(cleanPhone).filter(Boolean);
      const text = encodeURIComponent(msg);
  ...
  if(typeof buildCierreMsgAsync === "function" && !window.__cierreDef1592){
    window.__cierreDef1592 = true;
    const oldAsync = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      t = await hydrate(t);
      return normalizar(limpiar(await oldAsync.apply(this, arguments)),t);
    };
  }
  if(typeof buildCierreMsg === "function" && !window.__cierreSyncDef1592){
    window.__cierreSyncDef1592 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){
      return normalizar(limpiar(old.apply(this, arguments)),t);
    };
  }
  if(typeof cerrarTransito === "function" && !window.__cerrarTransitoDef1592){
    window.__cerrarTransitoDef1592 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      const t0 = typeof transit === "function" ? transit() : null;
      if(t0) await hydrate(t0);
      return await oldCerrar.apply(this, arguments);
    };
  }
  if(typeof sendToPhones === "function" && !window.__sendDef1592){
    window.__sendDef1592 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      const finalMsg = normalizarActual(msg);
      try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){}
      return oldSend.call(this, finalMsg);
    };
  }
  if(typeof openWhatsappSelector === "function" && !window.__openWaDef1592){
    window.__openWaDef1592 = true;
    const oldOpen = openWhatsappSelector;
    openWhatsappSelector = function(msg){
      return oldOpen.call(this, normalizarActual(msg));
    };
  }
  if(typeof renderUltimo === "function" && !window.__ultimoDef1592){
    window.__ultimoDef1592 = true;
    const oldUltimo = renderUltimo;
    renderUltimo = async function(){
      try{
        const t = typeof transit === "function" ? transit() : null;
        ...
        if(last && last.msg && t){
          await hydrate(t);
          last.msg = normalizar(last.msg,t);
          save(LS.last,last);
        }
      }catch(e){}
      return oldUltimo.apply(this, arguments);
  ...
  function limpiarVista(){
    try{
      document.querySelectorAll(".aduanaItem").forEach(item=>{
        item.querySelectorAll(".aduanaUltimo,.aduanaRegistroDato1584,.aduanaRegistroDato1585,.aduanaRegistroDato1586,.aduanaRegistroDato1587,.aduanaRegistroDato1588,.aduanaRegistroDato1589,.aduanaRegistroDato1591,.aduanaRegistroDato1592").forEach(e=>e.remove());
        Array.from(item.childNodes).forEach(n=>{
          if(n.nodeType===3 && /\d{2}\/\d{2}\s+\d{2}:\d{2}/.test(n.textContent||"")) n.remove();
        });
        Array.from(item.children).forEach(el=>{
          if(!el.classList.contains("aduanaName") && !el.classList.contains("aduanaBtn") && /\d{2}\/\d{2}\s+\d{2}:\d{2}/.test(el.textContent||"") && !String(el.className).includes("aduanaRegistroDato")) el.remove();
        });
      });
    }catch(e){}
  }
  function pintarVista(){
    try{
      ...
      document.querySelectorAll(".aduanaItem").forEach(item=>{
        ...
        const visible = String(nameEl.textContent||"").replace(/^\s*\d+\.\s*/,"").trim().toLowerCase();
        const r = u.find(x=>{
          const n = title(x.aduana || x.aduanaId || "").toLowerCase();
          return n && (visible.includes(n) || n.includes(visible));
        });
        if(!r) return;
        const info = document.createElement("div");
        info.className = "aduanaRegistroDato1592";
        info.textContent = fh(r);
        nameEl.insertAdjacentElement("afterend", info);
        const btn = item.querySelector(".aduanaBtn");
        if(btn) btn.textContent = "Registrar nuevo ingreso";
      });
  ...
  if(typeof renderAduanas === "function" && !window.__renderAduanasDef1592){
    window.__renderAduanasDef1592 = true;
    const oldRender = renderAduanas;
    renderAduanas = async function(){
      const t = typeof transit === "function" ? transit() : null;
      if(t) await hydrate(t);
      const r = await oldRender.apply(this, arguments);
      setTimeout(pintarVista,80);
      setTimeout(pintarVista,300);
      return r;
    };
  }
  document.addEventListener("DOMContentLoaded",()=>setTimeout(limpiarVista,500));
})();


/* ===== V1.5.103 - Cierre con Aduanas compacto y alerta procesando ===== */
(function(){
  ...
  function merge1593(a,b){
    ...
    return Object.values(m).sort((x,y)=>ms1593(x)-ms1593(y));
  }
  async function hydrate1593(t){
    if(!t) return t;
    if(typeof window.aduanasCompactoDef1592 === "object" && typeof window.aduanasCompactoDef1592.hydrate === "function"){
      try{ t = await window.aduanasCompactoDef1592.hydrate(t); }catch(e){}
    }
    let regs = Array.isArray(t.aduanas) ? t.aduanas.slice() : [];
    try{
      if(typeof firebaseReady === "function" && firebaseReady()){
        ...
        if(transitId){
          try{
            const s = await db.collection("aduana_registros").where("transitId","==",transitId).get();
            regs = merge1593(regs, s.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
        if(embarque && flota){
          try{
            const s2 = await db.collection("aduana_registros").where("embarque","==",embarque).where("flota","==",flota).get();
            regs = merge1593(regs, s2.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
      }
    }catch(e){}
    if(regs.length){
      t.aduanas = regs;
      try{ save(LS.transit,t); if(typeof saveTransitHistory === "function") saveTransitHistory(t); }catch(e){}
    }
    return t;
  }
  function ultimos1593(t){
    const regs = Array.isArray(t && t.aduanas) ? t.aduanas.slice() : [];
    const m = {};
    regs.forEach(r=>{
      const k = key1593(r);
      if(!k) return;
      if(!m[k] || ms1593(r) >= ms1593(m[k])) m[k]=r;
    });
    return Object.values(m).sort((a,b)=>ms1593(a)-ms1593(b));
  }
  function bloque1593(t){
    const regs = ultimos1593(t);
    if(!regs.length) return "Sin registros";
    return regs.map(r=>`• ${title1593(r.aduana || r.aduanaId || "Aduana")} - ${fh1593(r)}`).join("\n");
  }
  function limpiar1593(msg){
    return String(msg || "")
      .replace(/\n*🛃 Aduanas:[\s\S]*$/i,"")
      .replace(/\n*Aduanas:[\s\S]*$/i,"")
      .trimEnd();
  }
  function anexar1593(msg,t){
    return limpiar1593(msg) + `\n\n🛃 Aduanas:\n${bloque1593(t)}`;
  }

  window.__aduanasCompactoCierre1593 = { hydrate:hydrate1593, anexar:anexar1593, bloque:bloque1593 };

  /* Cierre: normalizar cualquier constructor de cierre */
  if(typeof buildCierreMsgAsync === "function" && !window.__buildCierreAduanas1593){
    window.__buildCierreAduanas1593 = true;
    const old = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      t = await hydrate1593(t);
      const base = await old.apply(this, arguments);
      return anexar1593(base,t);
    };
  }
  if(typeof buildCierreMsg === "function" && !window.__buildCierreSyncAduanas1593){
    window.__buildCierreSyncAduanas1593 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){
      return anexar1593(old.apply(this, arguments),t);
    };
  }

  /* Cierre: si la funcion arma/envia internamente, normalizar en sendToPhones */
  if(typeof sendToPhones === "function" && !window.__sendToPhonesAduanas1593){
    window.__sendToPhonesAduanas1593 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      let t = null;
      try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
      const finalMsg = t ? anexar1593(msg,t) : limpiar1593(msg);
      try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){}
      return oldSend.call(this, finalMsg);
    };
  }

  /* Cierre: hidratar antes de cerrar para que los registros esten disponibles */
  if(typeof cerrarTransito === "function" && !window.__cerrarHydrateAduanas1593){
    window.__cerrarHydrateAduanas1593 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      try{
        const t = typeof transit === "function" ? transit() : null;
        if(t) await hydrate1593(t);
      }catch(e){}
      return await oldCerrar.apply(this, arguments);
    };
  }

  /* Cartel compacto inmediato para Registrar alerta con GPS */
  function alertaBusy1593(show,msg){
    let box = document.getElementById("alertaBusy1593");
    if(show){
      if(!box){
        box = document.createElement("div");
        box.id = "alertaBusy1593";
        box.innerHTML = '<div class="alertaBusyCard1593"><div class="alertaBusySpin1593"></div><div id="alertaBusyText1593"></div></div>';
        document.body.appendChild(box);
      }
      const txt = document.getElementById("alertaBusyText1593");
  ...
  window.alertaBusy1593 = alertaBusy1593;

  function isRegistrarAlerta1593(el){
    const b = el && el.closest ? el.closest("button,input[type='button']") : null;
    if(!b) return false;
    const txt = String(b.textContent || b.value || "").toLowerCase();
    const on = String((b.getAttribute && b.getAttribute("onclick")) || "").toLowerCase();
    return (txt.includes("registrar alerta") && txt.includes("gps")) || on.includes("registraralerta");
  }

  document.addEventListener("click", function(ev){
    if(!isRegistrarAlerta1593(ev.target)) return;
    setTimeout(()=>alertaBusy1593(true,"Procesando..."),0);
  }, true);

  const posibles = ["registrarAlerta","registrarAlertaGps","registrarAlertaGPS","registrarAlertaConGps","registrarAlertaConGPS"];
  posibles.forEach(nombre=>{
    try{
      if(typeof window[nombre] === "function" && !window["__wrap_"+nombre+"_1593"]){
        window["__wrap_"+nombre+"_1593"] = true;
        const old = window[nombre];
        window[nombre] = async function(){
          alertaBusy1593(true,"Procesando...");
          try{
            const r = old.apply(this, arguments);
            if(r && typeof r.then === "function") return await r;
            return r;
          }finally{
            alertaBusy1593(false);
          }
        };
      }
    }catch(e){}
  ...
  /* Fallback: ocultar si el flujo viejo no devuelve promesa */
  document.addEventListener("click", function(ev){
    if(!isRegistrarAlerta1593(ev.target)) return;
    setTimeout(()=>alertaBusy1593(false),12000);
  }, true);
})();


/* ===== V1.5.103 - Cierre Aduanas definitivo + Procesando cierre ===== */
(function(){
  function tc1594(v){
    if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
    return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
  ...
  function merge1594(a,b){
    const m = {};
    (Array.isArray(a)?a:[]).concat(Array.isArray(b)?b:[]).forEach(r=>{
      if(!r) return;
      const key = [String(r.aduanaId||r.aduana||""), String(r.time||r.createdAt||""), String(r.embarque||""), String(r.flota||"")].join("|");
      m[key]=r;
    });
    return Object.values(m).sort((x,y)=>ms1594(x)-ms1594(y));
  }
  async function hydrate1594(t){
    if(!t) return t;
    let regs = Array.isArray(t.aduanas) ? t.aduanas.slice() : [];
    try{
      if(typeof firebaseReady === "function" && firebaseReady()){
        ...
        if(transitId){
          try{
            const s = await db.collection("aduana_registros").where("transitId","==",transitId).get();
            regs = merge1594(regs, s.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
        if(embarque && flota){
          try{
            const s2 = await db.collection("aduana_registros").where("embarque","==",embarque).where("flota","==",flota).get();
            regs = merge1594(regs, s2.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
      }
    }catch(e){}
  ...
  function ultimos1594(t){
    const regs = Array.isArray(t && t.aduanas) ? t.aduanas.slice() : [];
    const m = {};
    regs.forEach(r=>{
      const k = k1594(r);
      if(!k) return;
      if(!m[k] || ms1594(r) >= ms1594(m[k])) m[k]=r;
    });
    return Object.values(m).sort((a,b)=>ms1594(a)-ms1594(b));
  }
  function bloque1594(t){
    const regs = ultimos1594(t);
    if(!regs.length) return "Sin registros";
    return regs.map(r=>`• ${tc1594(r.aduana || r.aduanaId || "Aduana")} - ${fh1594(r)}`).join("\n");
  }
  function limpiar1594(msg){
    return String(msg || "")
      .replace(/\n*🛃 Aduanas:[\s\S]*$/i,"")
      .replace(/\n*Aduanas:[\s\S]*$/i,"")
      .trimEnd();
  }
  function anexar1594(msg,t){
    return limpiar1594(msg) + `\n\n🛃 Aduanas:\n${bloque1594(t)}`;
  }

  window.__aduanasCierreFinal1594 = {hydrate:hydrate1594, anexar:anexar1594, bloque:bloque1594};

  async function cierreBase1594(t){
    let base = "";
    if(typeof buildCierreMsgAsync === "function") base = await buildCierreMsgAsync(t);
    else if(typeof buildCierreMsg === "function") base = buildCierreMsg(t);
    else base = "🏁 Cierre de tránsito";
    return limpiar1594(base);
  }

  if(typeof buildCierreMsgAsync === "function" && !window.__buildCierre1594){
    window.__buildCierre1594 = true;
    const old = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      t = await hydrate1594(t);
      return anexar1594(await old.apply(this, arguments), t);
    };
  }
  if(typeof buildCierreMsg === "function" && !window.__buildCierreSync1594){
    window.__buildCierreSync1594 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){
      return anexar1594(old.apply(this, arguments), t);
    };
  }

  function cierreBusy1594(show,msg){
    let box = document.getElementById("cierreBusy1594");
    if(show){
      if(!box){
        box = document.createElement("div");
        box.id = "cierreBusy1594";
        box.innerHTML = '<div class="cierreBusyCard1594"><div class="cierreBusySpin1594"></div><div id="cierreBusyText1594"></div></div>';
        document.body.appendChild(box);
      }
      const txt = document.getElementById("cierreBusyText1594");
      if(txt) txt.textContent = msg || "Procesando...";
      box.style.display = "flex";
    }else if(box){
      box.style.display = "none";
  ...
  window.cierreBusy1594 = cierreBusy1594;

  function isCierreBtn1594(el){
    const b = el && el.closest ? el.closest("button,input[type='button']") : null;
    if(!b) return false;
    const txt = String(b.textContent || b.value || "").toLowerCase();
    const on = String((b.getAttribute && b.getAttribute("onclick")) || "").toLowerCase();
    return txt.includes("cierre tránsito") || txt.includes("cierre transito") || txt.includes("cerrar tránsito") || txt.includes("cerrar transito") || on.includes("cerrartransito");
  }

  document.addEventListener("click", function(ev){
    if(!isCierreBtn1594(ev.target)) return;
    setTimeout(()=>cierreBusy1594(true,"Procesando..."),0);
  }, true);

  /* Reemplazo de cierre: usa flujo original pero hidrata antes y normaliza cualquier envío */
  if(typeof cerrarTransito === "function" && !window.__cerrarTransitoFinal1594){
    window.__cerrarTransitoFinal1594 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      cierreBusy1594(true,"Procesando...");
      try{
        const t = typeof transit === "function" ? transit() : null;
        if(t) await hydrate1594(t);
        return await oldCerrar.apply(this, arguments);
      }finally{
        cierreBusy1594(false);
      }
    };
  }

  /* Normaliza cualquier WhatsApp, especialmente cierre generado dentro del flujo viejo */
  if(typeof sendToPhones === "function" && !window.__sendCierreFinal1594){
    window.__sendCierreFinal1594 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      let t = null;
      try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
      const finalMsg = t ? anexar1594(msg,t) : limpiar1594(msg);
      try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){}
      return oldSend.call(this, finalMsg);
    };
  }
  if(typeof openWhatsappSelector === "function" && !window.__openCierreFinal1594){
    window.__openCierreFinal1594 = true;
    const oldOpen = openWhatsappSelector;
    openWhatsappSelector = function(msg){
      let t = null;
      try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
      return oldOpen.call(this, t ? anexar1594(msg,t) : limpiar1594(msg));
    };
  }
})();


/* ===== V1.5.103 - Cierre Aduanas forzado antes de limpiar tránsito ===== */
(function(){
  function tc1595(v){
    if(typeof aduanaTitleCase1583 === "function") return aduanaTitleCase1583(v);
    return String(v||"").trim().split(/\s+/).map(w=>w ? w.charAt(0).toUpperCase()+w.slice(1).toLowerCase() : "").join(" ");
  ...
  function merge1595(a,b){
    ...
    return Object.values(m).sort((x,y)=>ms1595(x)-ms1595(y));
  }
  async function hydrate1595(t){
    if(!t) return t;
    let regs = Array.isArray(t.aduanas) ? t.aduanas.slice() : [];
    try{
      if(typeof firebaseReady === "function" && firebaseReady()){
        ...
        if(transitId){
          try{
            const s = await db.collection("aduana_registros").where("transitId","==",transitId).get();
            regs = merge1595(regs, s.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
        if(embarque && flota){
          try{
            const s2 = await db.collection("aduana_registros").where("embarque","==",embarque).where("flota","==",flota).get();
            regs = merge1595(regs, s2.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
      }
    }catch(e){}
    if(regs.length) t.aduanas = regs;
    return t;
  }
  function ultimos1595(t){
    const regs = Array.isArray(t && t.aduanas) ? t.aduanas.slice() : [];
    const m = {};
    regs.forEach(r=>{
      const k = key1595(r);
      if(!k) return;
      if(!m[k] || ms1595(r) >= ms1595(m[k])) m[k]=r;
    });
    return Object.values(m).sort((a,b)=>ms1595(a)-ms1595(b));
  }
  function bloque1595(t){
    const regs = ultimos1595(t);
    if(!regs.length) return "Sin registros";
    return regs.map(r=>`• ${tc1595(r.aduana || r.aduanaId || "Aduana")} - ${fh1595(r)}`).join("\n");
  }
  function limpiar1595(msg){
    return String(msg || "")
      .replace(/\n*🛃 Aduanas:[\s\S]*$/i,"")
      .replace(/\n*Aduanas:[\s\S]*$/i,"")
      .trimEnd();
  }
  function anexar1595(msg,t){
    return limpiar1595(msg) + `\n\n🛃 Aduanas:\n${bloque1595(t)}`;
  }
  window.__aduanasCierreFinal1595 = {hydrate:hydrate1595, anexar:anexar1595, bloque:bloque1595};

  async function mensajeCierre1595(t){
    t = await hydrate1595(t);
    let base = "";
    if(typeof buildCierreMsgAsync === "function") base = await buildCierreMsgAsync(t);
    else if(typeof buildCierreMsg === "function") base = buildCierreMsg(t);
    else base = "🏁 Cierre de tránsito";
    return anexar1595(base,t);
  }

  /* Guardar snapshot previo: el cierre viejo puede limpiar LS.transit antes de enviar */
  let cierreSnapshot1595 = null;

  if(typeof cerrarTransito === "function" && !window.__cerrarTransitoFinal1595){
    window.__cerrarTransitoFinal1595 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      try{
        cierreSnapshot1595 = typeof transit === "function" ? JSON.parse(JSON.stringify(transit())) : null;
        if(cierreSnapshot1595) cierreSnapshot1595 = await hydrate1595(cierreSnapshot1595);
        if(typeof cierreBusy1594 === "function") cierreBusy1594(true,"Procesando...");
      }catch(e){}
      try{
        return await oldCerrar.apply(this, arguments);
      }finally{
        setTimeout(()=>{ if(typeof cierreBusy1594 === "function") cierreBusy1594(false); },300);
      }
    };
  }

  /* Si el cierre envía por selector o sendToPhones, usar snapshot previo con aduanas */
  if(typeof sendToPhones === "function" && !window.__sendCierreFinal1595){
    window.__sendCierreFinal1595 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      const t = cierreSnapshot1595 || (typeof transit === "function" ? transit() : null);
      const finalMsg = t ? anexar1595(msg,t) : limpiar1595(msg);
      try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){}
      return oldSend.call(this, finalMsg);
    };
  }
  if(typeof openWhatsappSelector === "function" && !window.__openCierreFinal1595){
    window.__openCierreFinal1595 = true;
    const oldOpen = openWhatsappSelector;
    openWhatsappSelector = function(msg){
      const t = cierreSnapshot1595 || (typeof transit === "function" ? transit() : null);
      return oldOpen.call(this, t ? anexar1595(msg,t) : limpiar1595(msg));
    };
  }

  if(typeof buildCierreMsgAsync === "function" && !window.__buildCierreFinal1595){
    window.__buildCierreFinal1595 = true;
    const old = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      t = await hydrate1595(t || cierreSnapshot1595);
      const base = await old.apply(this, arguments);
      return anexar1595(base,t);
    };
  }
  if(typeof buildCierreMsg === "function" && !window.__buildCierreSyncFinal1595){
    window.__buildCierreSyncFinal1595 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){
      t = t || cierreSnapshot1595;
      return anexar1595(old.apply(this, arguments),t);
    };
  }
})();


/* ===== V1.5.103 - Correccion definitiva cierre Aduanas ===== */
(function(){
  ...
  function ms1596(r){ const d = new Date((r && (r.time || r.createdAt)) || ""); return isNaN(d.getTime()) ? 0 : d.getTime(); }
  function fh1596(r){
    const raw = (r && (r.time || r.createdAt)) || "";
    if(!raw) return "-";
    const d = new Date(raw);
  ...
  function key1596(r){ return String((r && (r.aduanaId || r.aduana)) || "").trim().toLowerCase(); }
  function merge1596(a,b){
    const m = {};
    (Array.isArray(a)?a:[]).concat(Array.isArray(b)?b:[]).forEach(r=>{
      if(!r) return;
      const k = [String(r.aduanaId||r.aduana||""), String(r.time||r.createdAt||""), String(r.embarque||""), String(r.flota||"")].join("|");
    ...
    return Object.values(m).sort((x,y)=>ms1596(x)-ms1596(y));
  }
  async function hydrate1596(t){
    if(!t) return t;
    let regs = Array.isArray(t.aduanas) ? t.aduanas.slice() : [];
    try{
      if(typeof firebaseReady === "function" && firebaseReady()){
        ...
        if(transitId){
          try{
            const s = await db.collection("aduana_registros").where("transitId","==",transitId).get();
            regs = merge1596(regs, s.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
        if(embarque && flota){
          try{
            const s2 = await db.collection("aduana_registros").where("embarque","==",embarque).where("flota","==",flota).get();
            regs = merge1596(regs, s2.docs.map(d=>({id:d.id,...(d.data()||{})})));
          }catch(e){}
        }
      }
    }catch(e){}
  ...
  function ultimos1596(t){
    const regs = Array.isArray(t && t.aduanas) ? t.aduanas.slice() : [];
    const m = {};
    regs.forEach(r=>{
      const k = key1596(r);
      if(!k) return;
      if(!m[k] || ms1596(r) >= ms1596(m[k])) m[k] = r;
    });
    return Object.values(m).sort((a,b)=>ms1596(a)-ms1596(b));
  }
  function bloque1596(t){
    const regs = ultimos1596(t);
    if(!regs.length) return "Sin registros";
    return regs.map(r => `• ${title1596(r.aduana || r.aduanaId || "Aduana")} - ${fh1596(r)}`).join("\n");
  }
  function limpiar1596(msg){
    return String(msg || "").replace(/\n*🛃 Aduanas:[\s\S]*$/i,"").replace(/\n*Aduanas:[\s\S]*$/i,"").trimEnd();
  }
  function anexar1596(msg,t){ return limpiar1596(msg) + `\n\n🛃 Aduanas:\n${bloque1596(t)}`; }
  window.__aduanas1596 = {hydrate:hydrate1596, anexar:anexar1596, bloque:bloque1596};

  let cierreSnapshot1596 = null;
  function capture1596(){
    try{
      const t = typeof transit === "function" ? transit() : null;
      cierreSnapshot1596 = t ? JSON.parse(JSON.stringify(t)) : cierreSnapshot1596;
      if(cierreSnapshot1596) hydrate1596(cierreSnapshot1596).then(x=>{ cierreSnapshot1596=x; });
    }catch(e){}
  }
  function isCierreBtn1596(el){
    const b = el && el.closest ? el.closest("button,input[type='button']") : null;
    if(!b) return false;
    const txt = String(b.textContent || b.value || "").toLowerCase();
    const on = String((b.getAttribute && b.getAttribute("onclick")) || "").toLowerCase();
  ...
  document.addEventListener("click", function(ev){
    if(!isCierreBtn1596(ev.target)) return;
    capture1596();
    try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(true,"Procesando..."); }catch(e){}
  }, true);

  if(typeof cerrarTransito === "function" && !window.__cerrarTransito1596){
    window.__cerrarTransito1596 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      capture1596();
      if(cierreSnapshot1596) cierreSnapshot1596 = await hydrate1596(cierreSnapshot1596);
      try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(true,"Procesando..."); }catch(e){}
      try{ return await oldCerrar.apply(this, arguments); }
      finally{ setTimeout(()=>{ try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(false); }catch(e){} },500); }
    };
  }
  if(typeof buildCierreMsgAsync === "function" && !window.__buildCierre1596){
    window.__buildCierre1596 = true;
    const old = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){ t = await hydrate1596(t || cierreSnapshot1596); return anexar1596(await old.apply(this, arguments), t); };
  }
  if(typeof buildCierreMsg === "function" && !window.__buildCierreSync1596){
    window.__buildCierreSync1596 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){ t = t || cierreSnapshot1596; return anexar1596(old.apply(this, arguments), t); };
  }
  function normalizarSalida1596(msg){
    let t = null;
    try{ t = cierreSnapshot1596 || (typeof transit === "function" ? transit() : null); }catch(e){}
    return t ? anexar1596(msg,t) : limpiar1596(msg);
  }
  if(typeof sendToPhones === "function" && !window.__send1596){
    window.__send1596 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){ const finalMsg = normalizarSalida1596(msg); try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){} return oldSend.call(this, finalMsg); };
  }
  if(typeof openWhatsappSelector === "function" && !window.__openWa1596){
    window.__openWa1596 = true;
    const oldOpen = openWhatsappSelector;
    openWhatsappSelector = function(msg){ return oldOpen.call(this, normalizarSalida1596(msg)); };
  }
  function arreglarVista1596(){
    try{
      document.querySelectorAll(".aduanaName").forEach(el=>{
        const m = String(el.textContent || "").match(/^(\s*\d+\.\s*)(.*)$/);
        if(m) el.textContent = m[1] + title1596(m[2]);
        else el.textContent = title1596(el.textContent);
      });
    }catch(e){}
  }
  if(typeof renderAduanas === "function" && !window.__renderAduanas1596){
    window.__renderAduanas1596 = true;
    const oldRender = renderAduanas;
    renderAduanas = async function(){ const r = await oldRender.apply(this, arguments); setTimeout(arreglarVista1596,80); setTimeout(arreglarVista1596,300); return r; };
  }
})();


/* ===== V1.5.103 - Fix definitivo cierre Aduanas + inicio no bloqueado ===== */
(function(){
  function tc(v){
    const s = String(v||"").trim();
    return s.split(/\s+/).map(w=>{
  ...
  function k(r){ return String((r && (r.aduanaId || r.aduana)) || "").trim().toLowerCase(); }
  function merge(a,b){
    const m = {};
    (Array.isArray(a)?a:[]).concat(Array.isArray(b)?b:[]).forEach(r=>{
      if(!r) return;
  ...
  function ultimos(t){
    ...
    regs.forEach(r=>{
      const key = k(r);
      if(!key) return;
      if(!m[key] || ms(r) >= ms(m[key])) m[key] = r;
    });
    return Object.values(m).sort((a,b)=>ms(a)-ms(b));
  }
  function bloque(t){
  ...
  function anexar(msg,t){ return limpiar(msg) + `\n\n🛃 Aduanas:\n${bloque(t)}`; }
  window.__aduanas1597 = {hydrate, anexar, bloque};

  /* Snapshot global: se captura antes de que cerrarTransito limpie el tránsito */
  let cierreSnap = null;
  async function captureCierre(){
    try{
      const t = typeof transit === "function" ? transit() : null;
      if(t){
        cierreSnap = JSON.parse(JSON.stringify(t));
        cierreSnap = await hydrate(cierreSnap);
      }
    }catch(e){}
    return cierreSnap;
  }

  function isCierreBtn(el){
    const b = el && el.closest ? el.closest("button,input[type='button']") : null;
    if(!b) return false;
    const txt = String(b.textContent || b.value || "").toLowerCase();
    const on = String((b.getAttribute && b.getAttribute("onclick")) || "").toLowerCase();
  ...
  document.addEventListener("click", function(ev){
    if(!isCierreBtn(ev.target)) return;
    try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(true,"Procesando..."); }catch(e){}
    captureCierre();
  }, true);

  /* Normalizador para cualquier salida WhatsApp */
  async function normalizarAsync(msg){
    let t = cierreSnap;
    if(!t){
      try{ t = typeof transit === "function" ? transit() : null; }catch(e){}
      if(t) t = await hydrate(t);
    }
    return t ? anexar(msg,t) : limpiar(msg);
  }
  function normalizarSync(msg){
    let t = cierreSnap;
    if(!t){ try{ t = typeof transit === "function" ? transit() : null; }catch(e){} }
    return t ? anexar(msg,t) : limpiar(msg);
  }

  if(typeof buildCierreMsgAsync === "function" && !window.__buildCierre1597){
    window.__buildCierre1597 = true;
    const old = buildCierreMsgAsync;
    buildCierreMsgAsync = async function(t){
      const tt = await hydrate(t || cierreSnap);
      return anexar(await old.apply(this, arguments), tt);
    };
  }
  if(typeof buildCierreMsg === "function" && !window.__buildCierreSync1597){
    window.__buildCierreSync1597 = true;
    const old = buildCierreMsg;
    buildCierreMsg = function(t){
      return anexar(old.apply(this, arguments), t || cierreSnap);
    };
  }
  if(typeof cerrarTransito === "function" && !window.__cerrarTransito1597){
    window.__cerrarTransito1597 = true;
    const oldCerrar = cerrarTransito;
    cerrarTransito = async function(){
      try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(true,"Procesando..."); }catch(e){}
      await captureCierre();
      try{ return await oldCerrar.apply(this, arguments); }
      finally{ setTimeout(()=>{ try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(false); }catch(e){} },600); }
    };
  }
  if(typeof sendToPhones === "function" && !window.__send1597){
    window.__send1597 = true;
    const oldSend = sendToPhones;
    sendToPhones = function(msg){
      const finalMsg = normalizarSync(msg);
      try{ save(LS.last,{msg:finalMsg,date:now()}); }catch(e){}
      return oldSend.call(this, finalMsg);
    };
  }
  if(typeof openWhatsappSelector === "function" && !window.__open1597){
    window.__open1597 = true;
    const oldOpen = openWhatsappSelector;
    openWhatsappSelector = function(msg){ return oldOpen.call(this, normalizarSync(msg)); };
  }
  /* Muchos cierres usan location.href directo. Interceptar clicks de cierre y reescribir texto en el próximo tick */
  const oldAssign = window.location.assign ? window.location.assign.bind(window.location) : null;
  try{
    if(oldAssign && !window.__locAssign1597){
      window.__locAssign1597 = true;
      window.location.assign = function(url){
        try{
          const s = String(url||"");
          if((s.includes("wa.me") || s.includes("whatsapp.com/send")) && cierreSnap){
            const u = new URL(s);
            const txt = u.searchParams.get("text");
            if(txt){
              u.searchParams.set("text", encodeURIComponent(normalizarSync(decodeURIComponent(txt))));
              return oldAssign(u.toString());
            }
          }
        }catch(e){}
        return oldAssign(url);
  ...
  /* Iniciar tránsito: si falla validación, no dejar cartel colgado */
  function hideInicioBusy(){
    try{ if(typeof hideStartBusyMini === "function") hideStartBusyMini(); }catch(e){}
    try{
      const e = document.getElementById("startBusyMini");
      if(e) e.style.display = "none";
  ...
  if(typeof iniciarTransito === "function" && !window.__iniciarFix1597){
    window.__iniciarFix1597 = true;
    const oldIni = iniciarTransito;
    iniciarTransito = async function(){
      try{
        const r = oldIni.apply(this, arguments);
        if(r && typeof r.then === "function") return await r;
        return r;
      }catch(e){
        throw e;
  ...
  document.addEventListener("click", function(ev){
    const b = ev.target && ev.target.closest ? ev.target.closest("button,input[type='button']") : null;
    if(!b) return;
    const txt = String(b.textContent || b.value || "").toLowerCase();
    const on = String((b.getAttribute && b.getAttribute("onclick")) || "").toLowerCase();
    if(txt.includes("iniciar tránsito") || txt.includes("iniciar transito") || on.includes("iniciartransito")){
      setTimeout(hideInicioBusy, 12000);
    }
  }, true);

  /* Vista Aduanas: nombres con mayúsculas/minúsculas */
  function fixNames(){
    try{
      document.querySelectorAll(".aduanaName").forEach(el=>{
        const m = String(el.textContent || "").match(/^(\s*\d+\.\s*)(.*)$/);
        if(m) el.textContent = m[1] + tc(m[2]);
        else el.textContent = tc(el.textContent);
      });
    }catch(e){}
  }
  if(typeof renderAduanas === "function" && !window.__renderAduanas1597){
    window.__renderAduanas1597 = true;
    const oldRender = renderAduanas;
    renderAduanas = async function(){
      const r = await oldRender.apply(this, arguments);
      setTimeout(fixNames,80);
      setTimeout(fixNames,300);
      return r;
    };
  }
})();
...
/* ===== V1.5.103 - Control cartel inicio solo validado ===== */
(function(){
  const originalShow = window.showStartBusyMini;
  const originalHide = window.hideStartBusyMini;
  window.__allowStartBusy1598 = false;
  window.showStartBusyMini = function(){
    if(!window.__allowStartBusy1598) return;
    if(typeof originalShow === "function") return originalShow();
  };
  window.hideStartBusyMini = function(){
    window.__allowStartBusy1598 = false;
    if(typeof originalHide === "function") return originalHide();
    const e=document.getElementById("startBusyMini");
    if(e)e.style.display="none";
  };
  const oldIniciar = window.iniciarTransito;
  window.iniciarTransito = async function(){
    window.__allowStartBusy1598 = true;
    try{ return await oldIniciar.apply(this, arguments); }
    finally{
      setTimeout(()=>{ try{ window.hideStartBusyMini(); }catch(e){} },500);
    }
  };
})();


/* ===== V1.5.103 - No reescribir cierre sin snapshot ===== */
(function(){
  if(typeof sendToPhones === "function" && !window.__sendFinal1598){
    const old=sendToPhones;
    window.__sendFinal1598=true;
    sendToPhones=function(msg){
      // usar exactamente el mensaje ya armado por cerrarTransito; no reconstruir desde LS.transit vacío
      save(LS.last,{msg:String(msg||""),date:now()});
      const u=user();
      const phones=String(u.phones||"").split(/[,;\n\r]+/).map(cleanPhone).filter(Boolean);
      const text=encodeURIComponent(String(msg||""));
      if(phones.length>0){
...
/* ===== V1.5.103 - No reconstruir Aduanas ajenas ===== */
(function(){
...
/* ===== V1.5.103 - Vista Aduanas sólo tránsito actual ===== */
(function(){
  ...
  function ultimos1599(t){
    const regs=Array.isArray(t&&t.aduanas)?t.aduanas.slice():[];
    const m={};
    regs.forEach(r=>{
      const k=String((r.aduana||r.aduanaId)||"").trim().toLowerCase();
      if(!k)return;
      const d=new Date((r.time||r.createdAt)||"");
      const ms=isNaN(d.getTime())?0:d.getTime();
      if(!m[k]||ms>=m[k].ms)m[k]={r,ms};
    });
    return Object.values(m).map(x=>x.r);
  }
  function limpiarVista1599(){
    try{
      const t=typeof transit==="function"?transit():null;
      const ult=ultimos1599(t);
      document.querySelectorAll(".aduanaItem").forEach(item=>{
        item.querySelectorAll(".aduanaUltimo,.aduanaRegistroDato1584,.aduanaRegistroDato1585,.aduanaRegistroDato1586,.aduanaRegistroDato1587,.aduanaRegistroDato1588,.aduanaRegistroDato1589,.aduanaRegistroDato1591,.aduanaRegistroDato1592,.aduanaRegistroDato1596,.aduanaRegistroDato1599").forEach(e=>e.remove());
        const nameEl=item.querySelector(".aduanaName");
        if(!nameEl)return;
        const visible=String(nameEl.textContent||"").replace(/^\s*\d+\.\s*/,"").trim().toLowerCase();
        const r=ult.find(x=>{
          const n=String((x.aduana||x.aduanaId)||"").trim().toLowerCase();
          return n && (visible.includes(n)||n.includes(visible));
        });
        const btn=item.querySelector(".aduanaBtn");
        if(r){
          const info=document.createElement("div");
          info.className="aduanaRegistroDato1599";
          info.textContent=fecha1599(r);
          nameEl.insertAdjacentElement("afterend",info);
          if(btn)btn.textContent="Registrar nuevo ingreso";
        }else{
  ...
  if(typeof renderAduanas==="function" && !window.__renderAduanas1599){
    window.__renderAduanas1599=true;
    const old=renderAduanas;
    renderAduanas=async function(){
      const r=await old.apply(this,arguments);
      setTimeout(limpiarVista1599,90);
      setTimeout(limpiarVista1599,350);
      return r;
    };
  }
})();
...
/* ===== V1.5.103 - Aduanas/Cierre fuente única definitiva ===== */
(function(){
...
/* ===== V1.5.103 - Aduanas SOLO Firebase por transitId ===== */
(function(){
  function tc(v){
    const s=String(v||"").trim();
    return s.split(/\s+/).map(w=>{
...
/* ===== V1.5.103 - Aduanas sin refresh visual ===== */
(function(){
  /*
    Problema corregido:
    La vista Aduanas quedaba parpadeando porque al entrar se ejecutaban
    renders anteriores en paralelo y cada uno limpiaba/rearmaba la lista.
    Esta versión usa token de carga: sólo el último render puede pintar.
  */
  let aduanasRenderToken15102 = 0;
  let aduanasCache15102 = {
    transitId:"",
    html:"",
    estado:"",
    ts:0
  ...
  function ultimos15102(regs){
    const m={};
    (Array.isArray(regs)?regs:[]).forEach(r=>{
      const k=String((r&&(r.aduanaId||r.aduana))||"").trim().toLowerCase();
      if(!k)return;
      if(!m[k]||ms15102(r)>=ms15102(m[k]))m[k]=r;
    });
    return Object.values(m).sort((a,b)=>ms15102(a)-ms15102(b));
  }

  async function registrosFirebase15102(t){
    if(!t||!t.id)return [];
    try{
      if(!(typeof firebaseReady==="function" && firebaseReady()))return [];
      const snap=await db.collection("aduana_registros").where("transitId","==",String(t.id)).get();
      return ultimos15102(snap.docs.map(d=>({id:d.id,...(d.data()||{})})));
    }catch(e){
      console.log("Aduanas sin refresh: error leyendo Firebase",e);
      return [];
    }
  }

  function buildHtml15102(aduanas,regs){
    return aduanas.map((a,idx)=>{
      const rawId=String(a.id||a.nombre||a.aduana||"");
      const nombre=tc15102(rawId);
      const orden=typeof aduanaDisplayOrden==="function"?aduanaDisplayOrden(a,idx):(idx+1);
      const ultimo=regs.filter(r=>String(r.aduanaId||r.aduana)===rawId).slice(-1)[0];
      const fecha=ultimo?`<div class="aduanaRegistroDato15102">${fh15102(ultimo)}</div>`:"";
      const textoBoton=ultimo?"Registrar nuevo ingreso":"Registrar ingreso";
      return `<div class="aduanaItem">
        <div class="aduanaName">${orden}. ${escapeHtml(nombre)}</div>
        ${fecha}
  ...
  window.renderAduanas = async function(){
    ...
    if(!t||t.closed){
      if(estado)estado.innerText="Vista habilitada cuando hay tránsito iniciado.";
      list.innerHTML='<div class="emptyBox aduanaEmpty">Sin tránsito activo.</div>';
      aduanasCache15102={transitId:"",html:"",estado:"",ts:0};
      return;
    }

    const token=++aduanasRenderToken15102;
    const transitId=String(t.id||"");

    const destino=typeof aduanaDestinoNombre==="function"?aduanaDestinoNombre(t):"";
    const pais=typeof aduanaPaisDesdeDestino==="function"?await aduanaPaisDesdeDestino(t):"";
    const estadoTxt=pais?`Destino: ${tc15102(destino)} / País: ${tc15102(pais)}`:`Destino: ${tc15102(destino)} / País no informado.`;

    if(token!==aduanasRenderToken15102)return;

    if(estado)estado.innerText=estadoTxt;

    /*
      No borrar la lista si ya hay datos del mismo tránsito.
      Así evitamos el efecto se muestra/no se muestra durante la consulta.
    */
    if(aduanasCache15102.transitId===transitId && aduanasCache15102.html){
      list.innerHTML=aduanasCache15102.html;
    }else if(!list.innerHTML.trim()){
      list.innerHTML='<div class="emptyBox aduanaEmpty">Cargando aduanas...</div>';
    }

    const aduanas=typeof cargarAduanasPais==="function"?await cargarAduanasPais(pais):[];
    const regs=await registrosFirebase15102(t);

    if(token!==aduanasRenderToken15102)return;

    let html="";
    if(!aduanas.length){
      html='<div class="emptyBox aduanaEmpty">No hay aduanas activas para el país del destino.</div>';
    }else{
      html=buildHtml15102(aduanas,regs);
    }
  ...
  /*
    Al registrar ingreso: limpiar cache sólo al final y renderizar una vez.
    Evita doble repintado.
  */
  if(typeof window.registrarAduana==="function" && !window.__registrarAduanaSinRefresh15102){
    window.__registrarAduanaSinRefresh15102=true;
    const oldRegistrar=window.registrarAduana;
    window.registrarAduana=async function(){
      const r=await oldRegistrar.apply(this,arguments);
      aduanasCache15102={transitId:"",html:"",estado:"",ts:0};
      await window.renderAduanas();
      return r;
    };
...
/* ===== V1.5.103 - Vistas bloqueadas sin tránsito + WhatsApp cierre único ===== */
(function(){
  const vistasRequierenTransito15103 = new Set([
    "tracking",
    "aduanas",
    "embarques",
    "alertas",
  ...
  function hayTransitoAbierto15103(){
    try{
      const t = typeof transit === "function" ? transit() : null;
      return !!(t && !t.closed);
    }catch(e){
  ...
  function vistaId15103(id){
    id = String(id || "");
    const low = id.toLowerCase();
    if(low === "check-list" || low === "check_list" || low === "checklist") return "checklist";
    if(low === "último" || low === "ultimo" || low === "ultimoregistro") return "ultimo";
    return low;
  }

  function esVistaBloqueada15103(id){
    const v = vistaId15103(id);
    return vistasRequierenTransito15103.has(v);
  }

  function bloquearBotonesVistas15103(){
    const activo = hayTransitoAbierto15103();

    document.querySelectorAll("button,[role='button'],.tab,.pill,.navBtn").forEach(el=>{
      const txt = String(el.textContent || "").toLowerCase();
      const on = String((el.getAttribute && el.getAttribute("onclick")) || "").toLowerCase();
      const id = String(el.id || "").toLowerCase();

      const esDep =
        txt.includes("tracking") ||
        txt.includes("aduanas") ||
        txt.includes("embarques") ||
        ...
        id.includes("aduanas") ||
        id.includes("embarques") ||
        id.includes("alertas") ||
        id.includes("clima") ||
        id.includes("check") ||
        id.includes("ultimo") ||
        on.includes("tracking") ||
        on.includes("aduanas") ||
        on.includes("embarques") ||
        on.includes("alertas") ||
        on.includes("clima") ||
        on.includes("check") ||
        on.includes("ultimo");
      ...
      el.classList.toggle("vistaBloqueada15103", !activo);
      if(!activo){
  ...
  function mostrarBloqueo15103(){
    window.alert("Primero debe iniciar un tránsito para acceder a esta vista.");
  }

  if(typeof show === "function" && !window.__showBloqueo15103){
    window.__showBloqueo15103 = true;
    const oldShow15103 = show;
    show = function(id){
      if(esVistaBloqueada15103(id) && !hayTransitoAbierto15103()){
        bloquearBotonesVistas15103();
        mostrarBloqueo15103();
        return oldShow15103.call(this, "inicio");
      }
      const r = oldShow15103.apply(this, arguments);
      setTimeout(bloquearBotonesVistas15103, 30);
      return r;
    };
  }

  document.addEventListener("click", function(ev){
    const el = ev.target && ev.target.closest ? ev.target.closest("button,[role='button'],.tab,.pill,.navBtn") : null;
    if(!el) return;
    ...
    if(dep && !hayTransitoAbierto15103()){
      ev.preventDefault();
      ev.stopImmediatePropagation();
      bloquearBotonesVistas15103();
      mostrarBloqueo15103();
      try{ show("inicio"); }catch(e){}
    }
  }, true);

  function enviarWhatsappUnaVez15103(msg){
    if(window.__cerrandoWhatsapp15103) return;
    window.__cerrandoWhatsapp15103 = true;
    ...
    setTimeout(()=>{ window.__cerrandoWhatsapp15103 = false; }, 6000);
  }

  /*
    Reemplazo final de cierre para evitar doble apertura:
    - arma mensaje
    - guarda Firebase
    - limpia tránsito
    - envía WhatsApp UNA sola vez
    - muestra cartel final antes de volver a inicio
  */
  if(typeof cerrarTransito === "function" && !window.__cerrarUnico15103){
    window.__cerrarUnico15103 = true;
    cerrarTransito = async function(){
      const t = typeof transit === "function" ? transit() : null;
      if(!t){
        try{ if(typeof cierreBusy1594 === "function") cierreBusy1594(false); }catch(e){}
      ...
      try{
        ...
        let regs = [];
        try{
          if(window.__aduanasFirebaseOnly15101 && typeof window.__aduanasFirebaseOnly15101.registrosFirebaseTransitId === "function"){
            regs = await window.__aduanasFirebaseOnly15101.registrosFirebaseTransitId(t);
          }
        }catch(e){ regs = []; }

        let msg = await buildCierreMsgAsync(t);
        try{
          if(window.__aduanasFirebaseOnly15101 && typeof window.__aduanasFirebaseOnly15101.anexar === "function"){
            msg = window.__aduanasFirebaseOnly15101.anexar(msg, regs);
          }
        }catch(e){}
  ...
  if(typeof renderTransitStatus === "function" && !window.__renderTransitStatusBloqueo15103){
    window.__renderTransitStatusBloqueo15103 = true;
    const oldRenderTransitStatus15103 = renderTransitStatus;
    renderTransitStatus = function(){
      const r = oldRenderTransitStatus15103.apply(this, arguments);
      setTimeout(bloquearBotonesVistas15103, 40);
      return r;
    };
  }

  document.addEventListener("DOMContentLoaded",()=>{
    setTimeout(bloquearBotonesVistas15103,300);
    setTimeout(bloquearBotonesVistas15103,900);
  });
})();
